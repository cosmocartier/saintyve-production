...
import { AllItemsClient } from "@/components/category-pages/all-items/all-items-client"
...
export async function generateMetadata({
  ...
  searchParams: { brand?: string; subcategory?: string; color?: string; style?: string; sort?: string; search?: string }
}): Promise<Metadata> {
...
export default async function AllItemsPage({
  ...
  if (!allCategory) {
    console.error("All category not found")
    return <div>Error: All category not configured</div>
  }
  ...
  if (pcError) {
    console.error("Error fetching all items:", pcError)
  }
  ...
  const filterOptions = await getFilterOptionsForCategory(allCategory.slug)
          ...
          __html: JSON.stringify({
            ...
            itemListElement: [
              ...
              },
              {
                ...
                position: 2,
                name: "All Items",
                item: "https://designerdrip.store/all",
              },
            ],
          }),
        }}
