...
import { JewelryClient } from "@/components/category-pages/jewelry/jewelry-client"
...
export async function generateMetadata({
  ...
  return {
    title: "Designer Jewelry (AA+ Quality) | Designerdrip",
    description:
      "Shop premium designer jewelry with AA+ craftsmanship — rings, bracelets, necklaces & statement pieces from top labels. Fast worldwide shipping. New drops weekly.",
    openGraph: {
      title: "Designer Jewelry (AA+ Quality) | Designerdrip",
      description:
        "Shop premium designer jewelry with AA+ craftsmanship — rings, bracelets, necklaces & statement pieces from top labels. Fast worldwide shipping. New drops weekly.",
      type: "website",
      url: "https://designerdrip.com/jewelry",
    },
    twitter: {
    ...
    alternates: {
      canonical: "https://designerdrip.com/jewelry",
    },
    robots: {
...
export default async function JewelryPage({
  ...
  const { data: jewelryCategory } = await supabase
    .from("categories")
    .select("id, name, slug")
    .eq("slug", "jewelry")
    .single()
  ...
  if (!jewelryCategory) {
    console.error("Jewelry category not found")
    return <div>Error: Jewelry category not configured</div>
  }
  ...
  if (pcError) {
    console.error("Error fetching jewelry:", pcError)
  }
  ...
  const filterOptions = await getFilterOptionsForCategory(jewelryCategory.id)
          ...
          __html: JSON.stringify({
            ...
            itemListElement: [
              ...
              },
              {
                ...
                position: 2,
                name: "Jewelry",
                item: "https://designerdrip.store/jewelry",
              },
            ],
          }),
        }}
