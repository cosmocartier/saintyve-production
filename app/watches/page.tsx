...
import { WatchesClient } from "@/components/category-pages/watches/watches-client"
...
export async function generateMetadata({
  ...
  return {
    title: "Designer Watches (AA+ Quality) | Designerdrip",
    description:
      "Shop premium designer watches with AA+ craftsmanship — clean classics and statement pieces from top labels. Fast worldwide shipping. New drops weekly.",
    openGraph: {
      title: "Designer Watches (AA+ Quality) | Designerdrip",
      description:
        "Shop premium designer watches with AA+ craftsmanship — clean classics and statement pieces from top labels. Fast worldwide shipping. New drops weekly.",
      type: "website",
      url: "https://designerdrip.com/watches",
    },
    twitter: {
    ...
    alternates: {
      canonical: "https://designerdrip.com/watches",
    },
    robots: {
...
export default async function WatchesPage({
  ...
  const { data: watchesCategory } = await supabase
    .from("categories")
    .select("id, name, slug")
    .eq("slug", "watches")
    .single()
  ...
  if (!watchesCategory) {
    console.error("Watches category not found")
    return <div>Error: Watches category not configured</div>
  }
  ...
  let query = supabase
    ...
    `)
    .eq("category_id", watchesCategory.id)
  ...
  if (pcError) {
    console.error("Error fetching watches:", pcError)
  }
  ...
  const filterOptions = await getFilterOptionsForCategory(watchesCategory.id)
          ...
          __html: JSON.stringify({
            ...
            itemListElement: [
              ...
              },
              {
                ...
                position: 2,
                name: "Watches",
                item: "https://designerdrip.store/watches",
              },
            ],
          }),
        }}
