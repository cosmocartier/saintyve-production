...
import { JacketsClient } from "@/components/category-pages/jackets/jackets-client"
...
export async function generateMetadata({
  ...
  return {
    title: "Designer Jackets (AA+ Quality) | Designerdrip",
    description:
      "Shop premium designer jackets with AA+ craftsmanship — winter staples and statement outerwear from top labels. Fast worldwide shipping. New drops weekly.",
    openGraph: {
      title: "Designer Jackets (AA+ Quality) | Designerdrip",
      description:
        "Shop premium designer jackets with AA+ craftsmanship — winter staples and statement outerwear from top labels. Fast worldwide shipping. New drops weekly.",
      type: "website",
      url: "https://designerdrip.com/jackets",
    },
    twitter: {
    ...
    alternates: {
      canonical: "https://designerdrip.com/jackets",
    },
    robots: {
...
export default async function JacketsPage({
  ...
  const { data: jacketsCategory } = await supabase
    .from("categories")
    .select("id, name, slug")
    .eq("slug", "jackets")
    .single()
  ...
  if (!jacketsCategory) {
    console.error("Jackets category not found")
    return <div>Error: Jackets category not configured</div>
  }
  ...
  let query = supabase
    ...
    `)
    .eq("category_id", jacketsCategory.id)
  ...
  if (pcError) {
    console.error("Error fetching jackets:", pcError)
  }
  ...
  const filterOptions = await getFilterOptionsForCategory(jacketsCategory.id)
          ...
          __html: JSON.stringify({
            ...
            itemListElement: [
              ...
              },
              {
                ...
                position: 2,
                name: "Jackets",
                item: "https://designerdrip.store/jackets",
              },
            ],
          }),
        }}
