...
export async function PATCH(request: NextRequest, { params }: { params: { productId: string; imageId: string } }) {
  try {
    ...
    const { role, alt_text, sort_order } = body

    // If changing to primary or hover, unset previous one
    ...
    const updateData: { role?: string; alt_text?: string | null; sort_order?: number } = {}
    ...
    console.error("[CF Images] Update error:", error)
  ...
  try {
    ...
    if (imageData?.cf_image_id) {
      deleteFromCloudflare(imageData.cf_image_id).catch((err) => {
        console.error("[CF Images] Cloudflare delete error:", err)
      })
    }
