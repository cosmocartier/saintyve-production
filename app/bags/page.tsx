...
import { BagsClient } from "@/components/category-pages/bags/bags-client"
  ...
  return {
    ...
    openGraph: {
      ...
      description:
        "Shop premium designer bags with AA+ craftsmanship — crossbody, shoulder & travel pieces from top labels. Fast worldwide shipping. New drops weekly.",
      type: "website",
      url: "https://designerdrip.com/bags",
    },
    twitter: {
    ...
    alternates: {
      canonical: "https://designerdrip.com/bags",
    },
    robots: {
...
export default async function BagsPage({
  ...
  const { data: bagsCategory } = await supabase
    .from("categories")
    .select("id, name, slug")
    .eq("slug", "bags")
    .single()
  ...
  if (!bagsCategory) {
    console.error("Bags category not found")
    return <div>Error: Bags category not configured</div>
  }
  ...
  let query = supabase
    ...
    `)
    .eq("category_id", bagsCategory.id)
  ...
  if (pcError) {
    console.error("Error fetching bags:", pcError)
  }
  ...
  const filterOptions = await getFilterOptionsForCategory(bagsCategory.id)
          ...
          __html: JSON.stringify({
            ...
            itemListElement: [
              ...
              },
              {
                ...
                position: 2,
                name: "Bags",
                item: "https://designerdrip.store/bags",
              },
            ],
          }),
        }}
