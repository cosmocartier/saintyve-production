...
export function SneakersClient({ initialProducts, totalCount, initialLimit, categoryId, filterOptions, currentFilters }: SneakersClientProps) {
  ...
  useEffect(() => {
    const restoreState = async () => {
      const savedScrollPosition = sessionStorage.getItem("sneakersPageScrollPosition")
      const savedProductCount = sessionStorage.getItem("sneakersPageLoadedCount")
      ...
      if (savedProductCount && savedScrollPosition) {
        ...
        sessionStorage.removeItem("sneakersPageScrollPosition")
        sessionStorage.removeItem("sneakersPageLoadedCount")
      }
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      sessionStorage.setItem("sneakersPageScrollPosition", window.scrollY.toString())
      sessionStorage.setItem("sneakersPageLoadedCount", products.length.toString())
    }
  ...
  const buildFilterUrl = () => {
    ...
    return `/sneakers${params.toString() ? `?${params.toString()}` : ""}`
  }
    ...
    <div className="py-8">
      <div className="max-w-[1920px] mx-auto">
        ...
        <div className="mb-7 px-4">
          <h1 className="text-[15px] font-normal tracking-[0.15em] mb-6 text-[#111111] uppercase">SNEAKERS</h1>
        </div>

        <CategoryNavigationTabs currentCategory="sneakers" />

        {/* SEO Intro Block */}
        <div className="mb-10 px-4 max-w-4xl">
          <p className="text-[13px] leading-relaxed text-[#444444] tracking-wide text-zinc-400">
            Explore our curated collection of designer sneakers crafted with AA+ quality and precision. From iconic
            Jordan releases to premium Nike collaborations and Balenciaga statement pieces, each pair embodies superior
            craftsmanship and authentic styling. Our worldwide shipping ensures your new drops arrive swiftly and
            securely, while our weekly releases keep your rotation fresh with the latest from luxury fashion's most
            coveted brands.
          </p>
        </div>
        ...
        {sortedProducts.length === 0 ? (
          <div className="text-center py-16 px-8">
            <p className="text-gray-400 text-sm tracking-widest uppercase">NO SNEAKERS FOUND</p>
          </div>
        ) : (
          <>
