...
export function BagsClient({ initialProducts, totalCount, initialLimit, categoryId, filterOptions, currentFilters }: BagsClientProps) {
  ...
  useEffect(() => {
    const restoreState = async () => {
      const savedScrollPosition = sessionStorage.getItem("bagsPageScrollPosition")
      const savedProductCount = sessionStorage.getItem("bagsPageLoadedCount")
      ...
      if (savedProductCount && savedScrollPosition) {
        ...
        sessionStorage.removeItem("bagsPageScrollPosition")
        sessionStorage.removeItem("bagsPageLoadedCount")
      }
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      sessionStorage.setItem("bagsPageScrollPosition", window.scrollY.toString())
      sessionStorage.setItem("bagsPageLoadedCount", products.length.toString())
    }
  ...
  const buildFilterUrl = () => {
    ...
    return `/bags${params.toString() ? `?${params.toString()}` : ""}`
  }
    ...
    <div className="py-8">
      <div className="max-w-[1920px] mx-auto">
        ...
        <div className="mb-7 px-4">
          <h1 className="text-[15px] font-normal tracking-[0.15em] mb-6 text-[#111111] uppercase">BAGS</h1>
        </div>

        <CategoryNavigationTabs currentCategory="bags" />

        {/* SEO Intro Block */}
        <div className="mb-10 px-4 max-w-4xl">
          <p className="text-[13px] leading-relaxed text-[#444444] tracking-wide text-zinc-400">
            Explore our curated collection of designer bags crafted with AA+ quality and precision. From clean everyday
            crossbody pieces to statement shoulder bags and travel-ready silhouettes, each selection is chosen for sharp
            construction, premium materials, and refined detailing. Our weekly drops keep your rotation fresh, while
            worldwide shipping helps your next piece arrive quickly and securely. Discover elevated bags from luxury's
            most wanted labels — built to wear daily, style effortlessly, and hold up over time.
          </p>
        </div>
        ...
        {sortedProducts.length === 0 ? (
          <div className="text-center py-16 px-8">
            <p className="text-gray-400 text-sm tracking-widest uppercase">NO BAGS FOUND</p>
          </div>
        ) : (
          <>
