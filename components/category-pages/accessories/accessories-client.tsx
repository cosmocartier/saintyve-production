...
export function AccessoriesClient({ initialProducts, totalCount, initialLimit, categoryId, filterOptions, currentFilters }: AccessoriesClientProps) {
  ...
  useEffect(() => {
    const restoreState = async () => {
      const savedScrollPosition = sessionStorage.getItem("accessoriesPageScrollPosition")
      const savedProductCount = sessionStorage.getItem("accessoriesPageLoadedCount")
      ...
      if (savedProductCount && savedScrollPosition) {
        ...
        sessionStorage.removeItem("accessoriesPageScrollPosition")
        sessionStorage.removeItem("accessoriesPageLoadedCount")
      }
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      sessionStorage.setItem("accessoriesPageScrollPosition", window.scrollY.toString())
      sessionStorage.setItem("accessoriesPageLoadedCount", products.length.toString())
    }
  ...
  const buildFilterUrl = () => {
    ...
    return `/accessories${params.toString() ? `?${params.toString()}` : ""}`
  }
    ...
    <div className="py-8">
      <div className="max-w-[1920px] mx-auto">
        ...
        <div className="mb-7 px-4">
          <h1 className="text-[15px] font-normal tracking-[0.15em] mb-6 text-[#111111] uppercase">ACCESSORIES</h1>
        </div>

        {/* Category Tabs */}
        <CategoryNavigationTabs currentCategory="accessories" />

        {/* SEO Intro Block */}
        <div className="mb-10 px-4 max-w-4xl">
          <p className="text-[13px] leading-relaxed text-[#444444] tracking-wide text-zinc-400">
            Explore our curated collection of designer accessories crafted with AA+ quality and precision. From everyday
            essentials to statement finishing touches, each piece is chosen for clean construction, refined detailing,
            and a premium feel. Our weekly drops keep the lineup fresh, while worldwide shipping helps your next
            accessory arrive quickly and securely. Discover elevated accessories from luxury's most wanted labels —
            built to sharpen your fit, layer effortlessly, and add subtle impact without trying too hard.
          </p>
        </div>
        ...
        {sortedProducts.length === 0 ? (
          <div className="text-center py-16 px-8">
            <p className="text-gray-400 text-sm tracking-widest uppercase">NO ACCESSORIES FOUND</p>
          </div>
        ) : (
          <>
