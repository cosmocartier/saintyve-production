...
export function VestsClient({ initialProducts, totalCount, initialLimit, categoryId, filterOptions, currentFilters }: VestsClientProps) {
  ...
  useEffect(() => {
    const restoreState = async () => {
      const savedScrollPosition = sessionStorage.getItem("vestsPageScrollPosition")
      const savedProductCount = sessionStorage.getItem("vestsPageLoadedCount")
      ...
      if (savedProductCount && savedScrollPosition) {
        ...
        sessionStorage.removeItem("vestsPageScrollPosition")
        sessionStorage.removeItem("vestsPageLoadedCount")
      }
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      sessionStorage.setItem("vestsPageScrollPosition", window.scrollY.toString())
      sessionStorage.setItem("vestsPageLoadedCount", products.length.toString())
    }
  ...
  const buildFilterUrl = () => {
    ...
    return `/vests${params.toString() ? `?${params.toString()}` : ""}`
  }
    ...
    <div className="py-8">
      <div className="max-w-[1920px] mx-auto">
        <div className="mb-7 px-4">
          <h1 className="text-[15px] font-normal tracking-[0.15em] mb-6 text-[#111111] uppercase">VESTS</h1>
        </div>

        <CategoryNavigationTabs currentCategory="vests" />

        <div className="mb-10 px-4 max-w-4xl">
          <p className="text-[13px] leading-relaxed text-[#444444] tracking-wide">
            Vests offer a refined approach to layering, balancing functionality with understated design. Our selection
            focuses on premium materials and considered silhouettes suited for modern wardrobes.
          </p>
        </div>
        ...
        {sortedProducts.length === 0 ? (
          <div className="text-center py-16 px-8">
            <p className="text-gray-400 text-sm tracking-widest uppercase">NO VESTS FOUND</p>
          </div>
        ) : (
          <>
