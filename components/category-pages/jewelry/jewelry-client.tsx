...
export function JewelryClient({ initialProducts, totalCount, initialLimit, categoryId, filterOptions, currentFilters }: JewelryClientProps) {
  ...
  useEffect(() => {
    const restoreState = async () => {
      const savedScrollPosition = sessionStorage.getItem("jewelryPageScrollPosition")
      const savedProductCount = sessionStorage.getItem("jewelryPageLoadedCount")
      ...
      if (savedProductCount && savedScrollPosition) {
        ...
        sessionStorage.removeItem("jewelryPageScrollPosition")
        sessionStorage.removeItem("jewelryPageLoadedCount")
      }
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      sessionStorage.setItem("jewelryPageScrollPosition", window.scrollY.toString())
      sessionStorage.setItem("jewelryPageLoadedCount", products.length.toString())
    }
  ...
  const buildFilterUrl = () => {
    ...
    return `/jewelry${params.toString() ? `?${params.toString()}` : ""}`
  }
    ...
    <div className="py-8">
      <div className="max-w-[1920px] mx-auto">
        ...
        <div className="mb-7 px-4">
          <h1 className="text-[15px] font-normal tracking-[0.15em] mb-6 text-[#111111] uppercase">JEWELRY</h1>
        </div>

        {/* Category Tabs */}
        <CategoryNavigationTabs currentCategory="jewelry" />

        {/* SEO Intro Block */}
        <div className="mb-10 px-4 max-w-4xl">
          <p className="text-[13px] leading-relaxed text-[#444444] tracking-wide text-zinc-400">
            Explore our curated selection of designer jewelry crafted with AA+ quality and precision. From clean
            everyday pieces to bold statement silhouettes, each item is chosen for refined finishing, sharp detailing,
            and a premium feel. Our weekly drops keep the lineup fresh, while worldwide shipping helps your next piece
            arrive quickly and securely. Discover elevated jewelry from luxury's most wanted labels — designed to layer
            effortlessly, elevate any fit, and stand out with understated confidence.
          </p>
        </div>
        ...
        {sortedProducts.length === 0 ? (
          <div className="text-center py-16 px-8">
            <p className="text-gray-400 text-sm tracking-widest uppercase">NO JEWELRY FOUND</p>
          </div>
        ) : (
          <>
