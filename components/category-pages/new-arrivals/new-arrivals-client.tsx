...
export function NewArrivalsClient({ initialProducts, totalCount, initialLimit, categoryId, filterOptions, currentFilters }: NewArrivalsClientProps) {
  ...
  useEffect(() => {
    const restoreState = async () => {
      const savedScrollPosition = sessionStorage.getItem("newArrivalsPageScrollPosition")
      const savedProductCount = sessionStorage.getItem("newArrivalsPageLoadedCount")
      ...
      if (savedProductCount && savedScrollPosition) {
        ...
        sessionStorage.removeItem("newArrivalsPageScrollPosition")
        sessionStorage.removeItem("newArrivalsPageLoadedCount")
      }
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      sessionStorage.setItem("newArrivalsPageScrollPosition", window.scrollY.toString())
      sessionStorage.setItem("newArrivalsPageLoadedCount", products.length.toString())
    }
  ...
  const buildFilterUrl = () => {
    ...
    return `/new-arrivals${params.toString() ? `?${params.toString()}` : ""}`
  }
    ...
    <div className="py-8">
      <div className="max-w-[1920px] mx-auto">
        ...
        <div className="mb-7 px-4">
          <h1 className="text-[15px] font-normal tracking-[0.15em] mb-6 text-[#111111] uppercase">NEW ARRIVALS</h1>
        </div>

        <CategoryNavigationTabs currentCategory="new-arrivals" />
        ...
        {sortedProducts.length === 0 ? (
          <div className="text-center py-16 px-8">
            <p className="text-gray-400 text-sm tracking-widest uppercase">NO NEW ARRIVALS FOUND</p>
          </div>
        ) : (
          <>
