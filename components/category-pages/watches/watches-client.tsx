...
export function WatchesClient({ initialProducts, totalCount, initialLimit, categoryId, filterOptions, currentFilters }: WatchesClientProps) {
  ...
  useEffect(() => {
    const restoreState = async () => {
      const savedScrollPosition = sessionStorage.getItem("watchesPageScrollPosition")
      const savedProductCount = sessionStorage.getItem("watchesPageLoadedCount")
      ...
      if (savedProductCount && savedScrollPosition) {
        ...
        sessionStorage.removeItem("watchesPageScrollPosition")
        sessionStorage.removeItem("watchesPageLoadedCount")
      }
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      sessionStorage.setItem("watchesPageScrollPosition", window.scrollY.toString())
      sessionStorage.setItem("watchesPageLoadedCount", products.length.toString())
    }
  ...
  const buildFilterUrl = () => {
    ...
    return `/watches${params.toString() ? `?${params.toString()}` : ""}`
  }
    ...
    <div className="py-8">
      <div className="max-w-[1920px] mx-auto">
        ...
        <div className="mb-7 px-4">
          <h1 className="text-[15px] font-normal tracking-[0.15em] mb-6 text-[#111111] uppercase">WATCHES</h1>
        </div>

        <CategoryNavigationTabs currentCategory="watches" />

        {/* SEO Intro Block */}
        <div className="mb-10 px-4 max-w-4xl">
          <p className="text-[13px] leading-relaxed text-[#444444] tracking-wide text-zinc-400">
            Explore our curated selection of designer watches crafted with AA+ quality and precision. From clean
            everyday classics to bold statement pieces, each watch is chosen for refined detailing, solid finishing, and
            a sharp on-wrist look. Our weekly drops keep the lineup fresh, while worldwide shipping helps your next
            piece arrive quickly and securely. Discover elevated watches from luxury's most wanted labels — built to
            complement any rotation, wear effortlessly, and stand out with understated confidence.
          </p>
        </div>
        ...
        {sortedProducts.length === 0 ? (
          <div className="text-center py-16 px-8">
            <p className="text-gray-400 text-sm tracking-widest uppercase">NO WATCHES FOUND</p>
          </div>
        ) : (
          <>
