...
export function JacketsClient({ initialProducts, totalCount, initialLimit, categoryId, filterOptions, currentFilters }: JacketsClientProps) {
  ...
  useEffect(() => {
    const restoreState = async () => {
      const savedScrollPosition = sessionStorage.getItem("jacketsPageScrollPosition")
      const savedProductCount = sessionStorage.getItem("jacketsPageLoadedCount")
      ...
      if (savedProductCount && savedScrollPosition) {
        ...
        sessionStorage.removeItem("jacketsPageScrollPosition")
        sessionStorage.removeItem("jacketsPageLoadedCount")
      }
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      sessionStorage.setItem("jacketsPageScrollPosition", window.scrollY.toString())
      sessionStorage.setItem("jacketsPageLoadedCount", products.length.toString())
    }
  ...
  const buildFilterUrl = () => {
    ...
    return `/jackets${params.toString() ? `?${params.toString()}` : ""}`
  }
    ...
    <div className="py-8">
      <div className="max-w-[1920px] mx-auto">
        <div className="mb-7 px-4">
          <h1 className="text-[15px] font-normal tracking-[0.15em] mb-6 text-[#111111] uppercase">JACKETS</h1>
        </div>

        <CategoryNavigationTabs currentCategory="jackets" />

        <div className="mb-10 px-4 max-w-4xl">
          <p className="text-[13px] leading-relaxed text-[#444444] tracking-wide text-zinc-400">
            Explore our curated selection of designer jackets crafted with AA+ quality and precision. From cold-season
            staples to lightweight statement outerwear, each piece is chosen for clean construction, premium materials,
            and refined finishing. Our weekly drops keep your rotation fresh, while worldwide shipping helps your next
            jacket arrive quickly and securely. Discover elevated outerwear from luxury's most wanted labels — built for
            daily wear, sharp layering, and effortless styling across seasons.
          </p>
        </div>
        ...
        {sortedProducts.length === 0 ? (
          <div className="text-center py-16 px-8">
            <p className="text-gray-400 text-sm tracking-widest uppercase">NO JACKETS FOUND</p>
          </div>
        ) : (
          <>
