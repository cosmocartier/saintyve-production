...
interface WomensAccessoriesClientProps {
...
export function WomensAccessoriesClient({
  initialProducts,
  totalCount,
  initialLimit,
  categoryId,
  ...
  useEffect(() => {
    const restoreState = async () => {
      const savedScrollPosition = sessionStorage.getItem("womensAccessoriesPageScrollPosition")
      const savedProductCount = sessionStorage.getItem("womensAccessoriesPageLoadedCount")
      ...
      if (savedProductCount && savedScrollPosition) {
        ...
        sessionStorage.removeItem("womensAccessoriesPageScrollPosition")
        sessionStorage.removeItem("womensAccessoriesPageLoadedCount")
      }
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      sessionStorage.setItem("womensAccessoriesPageScrollPosition", window.scrollY.toString())
      sessionStorage.setItem("womensAccessoriesPageLoadedCount", products.length.toString())
    }
  ...
  const buildFilterUrl = () => {
    ...
    return `/womens/accessories${params.toString() ? `?${params.toString()}` : ""}`
  }
      ...
      <div className="max-w-[1920px] mx-auto">
        <div className="mb-6 px-4">
          ...
          <p className="text-[13px] text-[#111111] leading-relaxed mb-6">
            Explore our curated collection of women&apos;s accessories, featuring everything from statement belts and
            sunglasses to elegant jewelry and scarves. Each piece is carefully selected to elevate any look with
            timeless style.
          </p>
        </div>
          ...
          <>
            ...
            {hasMore && (
              <div ref={loadMoreTriggerRef} className="h-20 flex items-center justify-center">
                {/* Invisible trigger point for infinite scroll */}
              </div>
            )}
          </>
        )}
