...
interface WomensShoesClientProps {
...
export function WomensShoesClient({
  initialProducts,
  totalCount,
  initialLimit,
  categoryId,
  ...
  useEffect(() => {
    const restoreState = async () => {
      const savedScrollPosition = sessionStorage.getItem("womensShoesPageScrollPosition")
      const savedProductCount = sessionStorage.getItem("womensShoesPageLoadedCount")
      ...
      if (savedProductCount && savedScrollPosition) {
        ...
        sessionStorage.removeItem("womensShoesPageScrollPosition")
        sessionStorage.removeItem("womensShoesPageLoadedCount")
      }
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      sessionStorage.setItem("womensShoesPageScrollPosition", window.scrollY.toString())
      sessionStorage.setItem("womensShoesPageLoadedCount", products.length.toString())
    }
  ...
  const buildFilterUrl = () => {
    ...
    return `/womens/shoes${params.toString() ? `?${params.toString()}` : ""}`
  }
  ...
  // Sort products client-side as a visual preview (server already sorted, but this keeps things consistent when user changes sort before applying)
      ...
      <div className="max-w-[1920px] mx-auto">
        <div className="mb-6 px-4">
          ...
          <p className="text-[13px] text-[#111111] leading-relaxed mb-6">
            Explore our curated collection of women&apos;s footwear, featuring everything from classic heels and elegant
            flats to contemporary sneakers and seasonal boots. Each pair is carefully selected to blend timeless style
            with modern comfort.
          </p>
        </div>
