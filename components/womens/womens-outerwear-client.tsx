...
interface WomensOuterwearClientProps {
...
export function WomensOuterwearClient({
  ...
  useEffect(() => {
    const restoreState = async () => {
      const savedScrollPosition = sessionStorage.getItem("womensOuterwearPageScrollPosition")
      const savedProductCount = sessionStorage.getItem("womensOuterwearPageLoadedCount")
      ...
      if (savedProductCount && savedScrollPosition) {
        ...
        if (targetCount > initialProducts.length) {
          ...
          try {
            ...
            console.error("[v0] Error restoring outerwear state:", err)
          }
        }
        ...
        sessionStorage.removeItem("womensOuterwearPageScrollPosition")
        sessionStorage.removeItem("womensOuterwearPageLoadedCount")
      }
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      sessionStorage.setItem("womensOuterwearPageScrollPosition", window.scrollY.toString())
      sessionStorage.setItem("womensOuterwearPageLoadedCount", products.length.toString())
    }
  ...
  const buildFilterUrl = () => {
    ...
    return `/womens/outerwear${params.toString() ? `?${params.toString()}` : ""}`
  }
  ...
  const loadMoreProducts = async () => {
    ...
    try {
      ...
      console.error("[v0] Exception loading more outerwear products:", err)
    } finally {
      setIsLoading(false)
    }
  }
      ...
      <div className="max-w-[1920px] mx-auto">
        <div className="mb-6 px-4">
          ...
          <p className="text-[13px] text-[#111111] leading-relaxed mb-6">
            Explore our curated collection of women&apos;s outerwear, featuring everything from luxurious coats and
            tailored blazers to contemporary jackets and seasonal layers. Each piece is carefully selected to blend
