...
interface CesarePaciottiClientProps {
...
export function CesarePaciottiClient({
  initialProducts,
  totalCount,
  initialLimit,
  currentCategory,
  ...
  const buildFilterUrl = (type: "category" | "sort", value: string) => {
    ...
    return `/brands/cesare-paciotti${params.toString() ? `?${params.toString()}` : ""}`
  }

  useEffect(() => {
    const restoreState = async () => {
      const savedScrollPosition = sessionStorage.getItem("cesarePaciottiScrollPosition")
      const savedProductCount = sessionStorage.getItem("cesarePaciottiLoadedCount")
      ...
      if (savedProductCount && savedScrollPosition) {
        ...
        if (targetCount > initialProducts.length) {
          ...
          try {
            ...
            console.error("[v0] Error restoring Cesare Paciotti products:", error)
          }
        }
        ...
        sessionStorage.removeItem("cesarePaciottiScrollPosition")
        sessionStorage.removeItem("cesarePaciottiLoadedCount")
      }
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      sessionStorage.setItem("cesarePaciottiScrollPosition", window.scrollY.toString())
      sessionStorage.setItem("cesarePaciottiLoadedCount", products.length.toString())
    }
  ...
  const loadMoreProducts = async () => {
    ...
    try {
      ...
      if (error) {
        console.error("[v0] Error loading more Cesare Paciotti products:", error)
        return
      }
      ...
      console.error("[v0] Exception loading more Cesare Paciotti products:", error)
    } finally {
      setIsLoading(false)
    }
  }
      ...
      <section className="w-full max-w-[1920px] mx-auto mb-16">
        <div className="grid lg:grid-cols-2">
          ...
          <div className="bg-[#000000] px-8 py-12 lg:px-16 lg:py-16 flex flex-col justify-center">
            <h1 className="text-white text-4xl lg:text-5xl font-bold tracking-tight uppercase mb-6">CESARE PACIOTTI</h1>
            <p className="text-white/90 text-base lg:text-lg leading-relaxed max-w-xl">
              Founded in 1958, Cesare Paciotti embodies the pinnacle of Italian luxury footwear craftsmanship. Renowned for its iconic dagger logo and rebellious spirit, the brand combines traditional artisanal techniques with contemporary design. Each piece is meticulously handcrafted in Italy, featuring premium materials and audacious detailing that has made Cesare Paciotti a favorite among fashion connoisseurs and rock stars alike.
            </p>
          </div>
