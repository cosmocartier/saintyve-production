...
export function RimowaClient({
  ...
  const buildFilterUrl = (type: "category" | "sort", value: string) => {
    ...
    return `/brands/rimowa${params.toString() ? `?${params.toString()}` : ""}`
  }

  useEffect(() => {
    const restoreState = async () => {
      const savedScrollPosition = sessionStorage.getItem("rimowaScrollPosition")
      const savedProductCount = sessionStorage.getItem("rimowaLoadedCount")
      ...
      if (savedProductCount && savedScrollPosition) {
        ...
        if (targetCount > initialProducts.length) {
          ...
          try {
            ...
            console.error("[v0] Error restoring Rimowa products:", error)
          }
        }
        ...
        sessionStorage.removeItem("rimowaScrollPosition")
        sessionStorage.removeItem("rimowaLoadedCount")
      }
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      sessionStorage.setItem("rimowaScrollPosition", window.scrollY.toString())
      sessionStorage.setItem("rimowaLoadedCount", products.length.toString())
    }
  ...
  const loadMoreProducts = async () => {
    ...
    try {
      ...
      if (error) {
        console.error("[v0] Error loading more Rimowa products:", error)
        return
      }
      ...
      console.error("[v0] Exception loading more Rimowa products:", error)
    } finally {
      setIsLoading(false)
    }
  }
      ...
      <section className="w-full max-w-[1920px] mx-auto mb-16">
        <div className="grid lg:grid-cols-2">
          ...
          <div className="bg-[#000000] px-8 py-12 lg:px-16 lg:py-16 flex flex-col justify-center">
            <h1 className="text-white text-4xl lg:text-5xl font-bold tracking-tight uppercase mb-6">RIMOWA</h1>
            <p className="text-white/90 text-base lg:text-lg leading-relaxed max-w-xl">
              Since 1898, Rimowa has redefined luxury travel with iconic German engineering and design. Known for their
              legendary grooved aluminum and innovative polycarbonate cases, Rimowa luggage represents the perfect
              fusion of heritage craftsmanship and modern innovation. From the Classic to the Essential collection, each
              piece embodies the spirit of sophisticated travel and timeless functionality.
            </p>
          </div>
