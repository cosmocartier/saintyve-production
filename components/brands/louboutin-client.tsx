...
interface LouboutinClientProps {
...
export function LouboutinClient({
  initialProducts,
  totalCount,
  initialLimit,
  currentCategory,
  ...
  const buildFilterUrl = (type: "category" | "sort", value: string) => {
    ...
    return `/brands/louboutin${params.toString() ? `?${params.toString()}` : ""}`
  }

  useEffect(() => {
    const restoreState = async () => {
      const savedScrollPosition = sessionStorage.getItem("louboutinScrollPosition")
      const savedProductCount = sessionStorage.getItem("louboutinLoadedCount")
      ...
      if (savedProductCount && savedScrollPosition) {
        ...
        if (targetCount > initialProducts.length) {
          ...
          try {
            ...
            console.error("[v0] Error restoring Louboutin products:", error)
          }
        }
        ...
        sessionStorage.removeItem("louboutinScrollPosition")
        sessionStorage.removeItem("louboutinLoadedCount")
      }
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      sessionStorage.setItem("louboutinScrollPosition", window.scrollY.toString())
      sessionStorage.setItem("louboutinLoadedCount", products.length.toString())
    }
  ...
  const loadMoreProducts = async () => {
    ...
    try {
      ...
      if (error) {
        console.error("[v0] Error loading more Louboutin products:", error)
        return
      }
      ...
      console.error("[v0] Exception loading more Louboutin products:", error)
    } finally {
      setIsLoading(false)
    }
  }
      ...
      <section className="w-full max-w-[1920px] mx-auto mb-16">
        <div className="grid lg:grid-cols-2">
          ...
          <div className="bg-[#000000] px-8 py-12 lg:px-16 lg:py-16 flex flex-col justify-center">
            <h1 className="text-white text-4xl lg:text-5xl font-bold tracking-tight uppercase mb-6">LOUBOUTIN</h1>
            <p className="text-white/90 text-base lg:text-lg leading-relaxed max-w-xl">
              Since 1991, Christian Louboutin has redefined luxury footwear with his iconic red-lacquered soles that have
              become synonymous with glamour and sophistication. From the legendary Pigalle pumps to statement-making
              boots and accessories, each creation embodies Parisian elegance and uncompromising craftsmanship. Explore a
              world where artistry meets desire, and every step makes a statement.
            </p>
          </div>
