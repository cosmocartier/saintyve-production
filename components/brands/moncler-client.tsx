...
export function MonclerClient({
  ...
  const buildFilterUrl = (type: "category" | "sort", value: string) => {
    ...
    return `/brands/moncler${params.toString() ? `?${params.toString()}` : ""}`
  }

  useEffect(() => {
    const restoreState = async () => {
      const savedScrollPosition = sessionStorage.getItem("monclerScrollPosition")
      const savedProductCount = sessionStorage.getItem("monclerLoadedCount")
      ...
      if (savedProductCount && savedScrollPosition) {
        ...
        if (targetCount > initialProducts.length) {
          ...
          try {
            ...
            console.error("[v0] Error restoring Moncler products:", error)
          }
        }
        ...
        sessionStorage.removeItem("monclerScrollPosition")
        sessionStorage.removeItem("monclerLoadedCount")
      }
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      sessionStorage.setItem("monclerScrollPosition", window.scrollY.toString())
      sessionStorage.setItem("monclerLoadedCount", products.length.toString())
    }
  ...
  const loadMoreProducts = async () => {
    ...
    try {
      ...
      if (error) {
        console.error("[v0] Error loading more Moncler products:", error)
        return
      }
      ...
      console.error("[v0] Exception loading more Moncler products:", error)
    } finally {
      setIsLoading(false)
    }
  }
      ...
      <section className="w-full max-w-[1920px] mx-auto mb-16">
        <div className="grid lg:grid-cols-2">
          ...
          <div className="bg-[#000000] px-8 py-12 lg:px-16 lg:py-16 flex flex-col justify-center">
            <h1 className="text-white text-4xl lg:text-5xl font-bold tracking-tight uppercase mb-6">MONCLER</h1>
            <p className="text-white/90 text-base lg:text-lg leading-relaxed max-w-xl">
              Founded in the French Alps in 1952, Moncler has evolved from an alpine specialist to a global luxury
              powerhouse. Renowned for exceptional down jackets and innovative technical wear, Moncler represents the
              perfect fusion of mountain heritage and haute couture. Each piece combines cutting-edge materials with
              timeless design, delivering unparalleled warmth and sophisticated style for the modern adventurer.
            </p>
          </div>
