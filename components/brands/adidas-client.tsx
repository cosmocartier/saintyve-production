...
export function AdidasClient({
  ...
  const buildFilterUrl = (type: "category" | "sort", value: string) => {
    ...
    return `/brands/adidas${params.toString() ? `?${params.toString()}` : ""}`
  }

  useEffect(() => {
    const restoreState = async () => {
      const savedScrollPosition = sessionStorage.getItem("adidasScrollPosition")
      const savedProductCount = sessionStorage.getItem("adidasLoadedCount")
      ...
      if (savedProductCount && savedScrollPosition) {
        ...
        if (targetCount > initialProducts.length) {
          ...
          try {
            ...
            console.error("[v0] Error restoring Adidas products:", error)
          }
        }
        ...
        sessionStorage.removeItem("adidasScrollPosition")
        sessionStorage.removeItem("adidasLoadedCount")
      }
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      sessionStorage.setItem("adidasScrollPosition", window.scrollY.toString())
      sessionStorage.setItem("adidasLoadedCount", products.length.toString())
    }
  ...
  const loadMoreProducts = async () => {
    ...
    try {
      ...
      if (error) {
        console.error("[v0] Error loading more Adidas products:", error)
        return
      }
      ...
      console.error("[v0] Exception loading more Adidas products:", error)
    } finally {
      setIsLoading(false)
    }
  }
      ...
      <section className="w-full max-w-[1920px] mx-auto mb-16">
        <div className="grid lg:grid-cols-2">
          ...
          <div className="bg-[#000000] px-8 py-12 lg:px-16 lg:py-16 flex flex-col justify-center">
            <h1 className="text-white text-4xl lg:text-5xl font-bold tracking-tight uppercase mb-6">ADIDAS</h1>
            <p className="text-white/90 text-base lg:text-lg leading-relaxed max-w-xl">
              Founded in 1949, Adidas has been at the forefront of athletic innovation for over seven decades. From the
              iconic three stripes to groundbreaking technologies like Boost and Primeknit, Adidas continues to push the
              boundaries of performance and style. Explore a legacy of excellence that spans from the world's biggest
              stages to the streets, with footwear and apparel that embody the spirit of sport and culture.
            </p>
          </div>
