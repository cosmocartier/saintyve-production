...
interface JimmyChooClientProps {
...
export function JimmyChooClient({
  initialProducts,
  totalCount,
  initialLimit,
  currentCategory,
  ...
  const buildFilterUrl = (type: "category" | "sort", value: string) => {
    ...
    return `/brands/jimmy-choo${params.toString() ? `?${params.toString()}` : ""}`
  }

  useEffect(() => {
    const restoreState = async () => {
      const savedScrollPosition = sessionStorage.getItem("jimmyChooScrollPosition")
      const savedProductCount = sessionStorage.getItem("jimmyChooLoadedCount")
      ...
      if (savedProductCount && savedScrollPosition) {
        ...
        if (targetCount > initialProducts.length) {
          ...
          try {
            ...
            console.error("[v0] Error restoring Jimmy Choo products:", error)
          }
        }
        ...
        sessionStorage.removeItem("jimmyChooScrollPosition")
        sessionStorage.removeItem("jimmyChooLoadedCount")
      }
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      sessionStorage.setItem("jimmyChooScrollPosition", window.scrollY.toString())
      sessionStorage.setItem("jimmyChooLoadedCount", products.length.toString())
    }
  ...
  const loadMoreProducts = async () => {
    ...
    try {
      ...
      if (error) {
        console.error("[v0] Error loading more Jimmy Choo products:", error)
        return
      }
      ...
      console.error("[v0] Exception loading more Jimmy Choo products:", error)
    } finally {
      setIsLoading(false)
    }
  }
      ...
      <section className="w-full max-w-[1920px] mx-auto mb-16">
        <div className="grid lg:grid-cols-2">
          ...
          <div className="bg-[#000000] px-8 py-12 lg:px-16 lg:py-16 flex flex-col justify-center">
            <h1 className="text-white text-4xl lg:text-5xl font-bold tracking-tight uppercase mb-6">JIMMY CHOO</h1>
            <p className="text-white/90 text-base lg:text-lg leading-relaxed max-w-xl">
              Founded in 1996, Jimmy Choo has become synonymous with luxury footwear and accessories. From red carpets to
              runways, the brand embodies glamour and sophistication with its iconic stilettos, statement handbags, and
              exquisite craftsmanship. Each piece is a celebration of modern femininity, combining cutting-edge design
              with timeless elegance to create fashion that empowers and captivates.
            </p>
          </div>
