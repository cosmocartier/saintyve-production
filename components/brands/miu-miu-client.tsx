...
export function MiuMiuClient({
  ...
  const buildFilterUrl = (type: "category" | "sort", value: string) => {
    ...
    return `/brands/miu-miu${params.toString() ? `?${params.toString()}` : ""}`
  }

  useEffect(() => {
    const restoreState = async () => {
      const savedScrollPosition = sessionStorage.getItem("miuMiuScrollPosition")
      const savedProductCount = sessionStorage.getItem("miuMiuLoadedCount")
      ...
      if (savedProductCount && savedScrollPosition) {
        ...
        if (targetCount > initialProducts.length) {
          ...
          try {
            ...
            console.error("[v0] Error restoring Miu Miu products:", error)
          }
        }
        ...
        sessionStorage.removeItem("miuMiuScrollPosition")
        sessionStorage.removeItem("miuMiuLoadedCount")
      }
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      sessionStorage.setItem("miuMiuScrollPosition", window.scrollY.toString())
      sessionStorage.setItem("miuMiuLoadedCount", products.length.toString())
    }
  ...
  const loadMoreProducts = async () => {
    ...
    try {
      ...
      if (error) {
        console.error("[v0] Error loading more Miu Miu products:", error)
        return
      }
      ...
      console.error("[v0] Exception loading more Miu Miu products:", error)
    } finally {
      setIsLoading(false)
    }
  }
      ...
      <section className="w-full max-w-[1920px] mx-auto mb-16">
        <div className="grid lg:grid-cols-2">
          ...
          <div className="bg-[#000000] px-8 py-12 lg:px-16 lg:py-16 flex flex-col justify-center">
            <h1 className="text-white text-4xl lg:text-5xl font-bold tracking-tight uppercase mb-6">MIU MIU</h1>
            <p className="text-white/90 text-base lg:text-lg leading-relaxed max-w-xl">
              Established in 1993 by Miuccia Prada, Miu Miu embodies the playful rebellion of contemporary Italian
              luxury. As Prada's younger, more daring sister brand, Miu Miu explores fashion's experimental edge with
              bold silhouettes, unexpected details, and a distinctive sense of whimsy. From coveted bags and shoes to
              avant-garde ready-to-wear, each piece celebrates individuality and fearless self-expression.
            </p>
          </div>
