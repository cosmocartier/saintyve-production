...
export function RolexClient({
  ...
  const buildFilterUrl = (type: "category" | "sort", value: string) => {
    ...
    return `/brands/rolex${params.toString() ? `?${params.toString()}` : ""}`
  }

  useEffect(() => {
    const restoreState = async () => {
      const savedScrollPosition = sessionStorage.getItem("rolexScrollPosition")
      const savedProductCount = sessionStorage.getItem("rolexLoadedCount")
      ...
      if (savedProductCount && savedScrollPosition) {
        ...
        if (targetCount > initialProducts.length) {
          ...
          try {
            ...
            console.error("[v0] Error restoring Rolex products:", error)
          }
        }
        ...
        sessionStorage.removeItem("rolexScrollPosition")
        sessionStorage.removeItem("rolexLoadedCount")
      }
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      sessionStorage.setItem("rolexScrollPosition", window.scrollY.toString())
      sessionStorage.setItem("rolexLoadedCount", products.length.toString())
    }
  ...
  const loadMoreProducts = async () => {
    ...
    try {
      ...
      if (error) {
        console.error("[v0] Error loading more Rolex products:", error)
        return
      }
      ...
      console.error("[v0] Exception loading more Rolex products:", error)
    } finally {
      setIsLoading(false)
    }
  }
      ...
      <section className="w-full max-w-[1920px] mx-auto mb-16">
        <div className="grid lg:grid-cols-2">
          ...
          <div className="bg-[#000000] px-8 py-12 lg:px-16 lg:py-16 flex flex-col justify-center">
            <h1 className="text-white text-4xl lg:text-5xl font-bold tracking-tight uppercase mb-6">ROLEX</h1>
            <p className="text-white/90 text-base lg:text-lg leading-relaxed max-w-xl">
              Since 1905, Rolex has epitomized Swiss excellence in luxury watchmaking. From the depths of the ocean with the Submariner to the peaks of mountains with the Explorer, each Rolex timepiece represents unparalleled precision, reliability, and timeless design. Discover iconic collections that have become symbols of achievement and prestige across generations—watches that don't just tell time, they tell stories of human excellence.
            </p>
          </div>
