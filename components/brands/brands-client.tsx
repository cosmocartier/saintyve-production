...
export function BrandsClient({
  ...
  const handleCategoryChange = (categorySlug: string) => {
    ...
    router.push(`/brands/${categorySlug}`, { scroll: false })
  }

  useEffect(() => {
    const restoreState = async () => {
      const savedScrollPosition = sessionStorage.getItem("brandsPageScrollPosition")
      const savedProductCount = sessionStorage.getItem("brandsPageLoadedCount")
      const savedCategorySlug = sessionStorage.getItem("brandsPageCategorySlug")

      if (savedProductCount && savedScrollPosition && savedCategorySlug) {
        const targetCount = Number.parseInt(savedProductCount, 10)
        const scrollY = Number.parseInt(savedScrollPosition, 10)

        if (savedCategorySlug !== selectedCategorySlug) {
        ...
        sessionStorage.removeItem("brandsPageScrollPosition")
        sessionStorage.removeItem("brandsPageLoadedCount")
        sessionStorage.removeItem("brandsPageCategorySlug")
      }
    ...
    restoreState()
  }, [initialProducts.length, selectedCategorySlug])

  useEffect(() => {
    const handleBeforeUnload = () => {
      sessionStorage.setItem("brandsPageScrollPosition", window.scrollY.toString())
      sessionStorage.setItem("brandsPageLoadedCount", products.length.toString())
      sessionStorage.setItem("brandsPageCategorySlug", selectedCategorySlug)
    }
  ...
  }, [products.length, selectedCategorySlug])
  ...
  const loadMoreProducts = async () => {
    ...
    try {
      ...
      if (!category) {
        console.error("[v0] Category not found")
        setIsLoading(false)
        return
      }
  ...
  const currentCategoryName = categories.find((c) => c.slug === selectedCategorySlug)?.name.toUpperCase() || "BRANDS"
