...
interface CanadaGooseClientProps {
...
export function CanadaGooseClient({
  initialProducts,
  totalCount,
  initialLimit,
  currentCategory,
  ...
  const buildFilterUrl = (type: "category" | "sort", value: string) => {
    ...
    return `/brands/canada-goose${params.toString() ? `?${params.toString()}` : ""}`
  }

  useEffect(() => {
    const restoreState = async () => {
      const savedScrollPosition = sessionStorage.getItem("canadaGooseScrollPosition")
      const savedProductCount = sessionStorage.getItem("canadaGooseLoadedCount")
      ...
      if (savedProductCount && savedScrollPosition) {
        ...
        if (targetCount > initialProducts.length) {
          ...
          try {
            ...
            console.error("[v0] Error restoring Canada Goose products:", error)
          }
        }
        ...
        sessionStorage.removeItem("canadaGooseScrollPosition")
        sessionStorage.removeItem("canadaGooseLoadedCount")
      }
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      sessionStorage.setItem("canadaGooseScrollPosition", window.scrollY.toString())
      sessionStorage.setItem("canadaGooseLoadedCount", products.length.toString())
    }
  ...
  const loadMoreProducts = async () => {
    ...
    try {
      ...
      if (error) {
        console.error("[v0] Error loading more Canada Goose products:", error)
        return
      }
      ...
      console.error("[v0] Exception loading more Canada Goose products:", error)
    } finally {
      setIsLoading(false)
    }
  }
      ...
      <section className="w-full max-w-[1920px] mx-auto mb-16">
        <div className="grid lg:grid-cols-2">
          ...
          <div className="bg-[#000000] px-8 py-12 lg:px-16 lg:py-16 flex flex-col justify-center">
            <h1 className="text-white text-4xl lg:text-5xl font-bold tracking-tight uppercase mb-6">CANADA GOOSE</h1>
            <p className="text-white/90 text-base lg:text-lg leading-relaxed max-w-xl">
              Founded in 1957 in a small warehouse in Toronto, Canada Goose has become synonymous with uncompromising quality and 
              extreme weather protection. From the frozen tundra of the Arctic to the peaks of Mount Everest, Canada Goose parkas 
              and outerwear deliver legendary warmth and durability. Each piece combines authentic Canadian craftsmanship with 
              timeless design, making it the choice of explorers, film crews, and style-conscious individuals worldwide.
            </p>
          </div>
