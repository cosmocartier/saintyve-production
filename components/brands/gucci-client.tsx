...
export function GucciClient({
  ...
  const buildFilterUrl = (type: "category" | "sort", value: string) => {
    ...
    return `/brands/gucci${params.toString() ? `?${params.toString()}` : ""}`
  }

  useEffect(() => {
    const restoreState = async () => {
      const savedScrollPosition = sessionStorage.getItem("gucciScrollPosition")
      const savedProductCount = sessionStorage.getItem("gucciLoadedCount")
      ...
      if (savedProductCount && savedScrollPosition) {
        ...
        if (targetCount > initialProducts.length) {
          ...
          try {
            ...
            console.error("[v0] Error restoring Gucci products:", error)
          }
        }
        ...
        sessionStorage.removeItem("gucciScrollPosition")
        sessionStorage.removeItem("gucciLoadedCount")
      }
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      sessionStorage.setItem("gucciScrollPosition", window.scrollY.toString())
      sessionStorage.setItem("gucciLoadedCount", products.length.toString())
    }
  ...
  const loadMoreProducts = async () => {
    ...
    try {
      ...
      if (error) {
        console.error("[v0] Error loading more Gucci products:", error)
        return
      }
      ...
      console.error("[v0] Exception loading more Gucci products:", error)
    } finally {
      setIsLoading(false)
    }
  }
      ...
      <section className="w-full max-w-[1920px] mx-auto mb-16">
        <div className="grid lg:grid-cols-2">
          ...
          <div className="bg-[#000000] px-8 py-12 lg:px-16 lg:py-16 flex flex-col justify-center">
            <h1 className="text-white text-4xl lg:text-5xl font-bold tracking-tight uppercase mb-6">GUCCI</h1>
            <p className="text-white/90 text-base lg:text-lg leading-relaxed max-w-xl">
              Founded in Florence in 1921, Gucci represents the pinnacle of Italian luxury and craftsmanship. From the iconic double G logo to the signature green-red-green web stripe, Gucci's designs blend heritage with contemporary innovation. Under Alessandro Michele's creative vision and now Sabato De Sarno, the house continues to set global trends with bold maximalism, eclectic patterns, and timeless elegance that defines modern luxury fashion.
            </p>
          </div>
