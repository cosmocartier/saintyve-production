...
export function VanCleefClient({
  initialProducts,
  totalCount,
  initialLimit,
  currentCategory,
  ...
  const buildFilterUrl = (type: "category" | "sort", value: string) => {
    ...
    return `/brands/van-cleef${params.toString() ? `?${params.toString()}` : ""}`
  }

  useEffect(() => {
    const restoreState = async () => {
      const savedScrollPosition = sessionStorage.getItem("vanCleefScrollPosition")
      const savedProductCount = sessionStorage.getItem("vanCleefLoadedCount")
      ...
      if (savedProductCount && savedScrollPosition) {
        ...
        if (targetCount > initialProducts.length) {
          ...
          try {
            ...
            console.error("[v0] Error restoring Van Cleef products:", error)
          }
        }
        ...
        sessionStorage.removeItem("vanCleefScrollPosition")
        sessionStorage.removeItem("vanCleefLoadedCount")
      }
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      sessionStorage.setItem("vanCleefScrollPosition", window.scrollY.toString())
      sessionStorage.setItem("vanCleefLoadedCount", products.length.toString())
    }
  ...
  const loadMoreProducts = async () => {
    ...
    try {
      ...
      if (error) {
        console.error("[v0] Error loading more Van Cleef products:", error)
        return
      }
      ...
      console.error("[v0] Exception loading more Van Cleef products:", error)
    } finally {
      setIsLoading(false)
    }
  }
      ...
      <section className="w-full max-w-[1920px] mx-auto mb-16">
        <div className="grid lg:grid-cols-2">
          ...
          <div className="bg-[#000000] px-8 py-12 lg:px-16 lg:py-16 flex flex-col justify-center">
            <h1 className="text-white text-4xl lg:text-5xl font-bold tracking-tight uppercase mb-6">VAN CLEEF & ARPELS</h1>
            <p className="text-white/90 text-base lg:text-lg leading-relaxed max-w-xl">
              Founded in 1906 in Paris, Van Cleef & Arpels represents the pinnacle of French haute joaillerie. 
              Renowned for the iconic Alhambra collection, Mystery Set technique, and exceptional savoir-faire, the 
              Maison has adorned royalty and celebrities for over a century. Each creation embodies poetic beauty, 
              transforming precious stones into enchanting symbols of love, nature, and timeless elegance.
            </p>
          </div>
