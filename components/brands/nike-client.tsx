...
export function NikeClient({
  ...
  const buildFilterUrl = (type: "category" | "sort", value: string) => {
    ...
    return `/brands/nike${params.toString() ? `?${params.toString()}` : ""}`
  }

  useEffect(() => {
    const restoreState = async () => {
      const savedScrollPosition = sessionStorage.getItem("nikeScrollPosition")
      const savedProductCount = sessionStorage.getItem("nikeLoadedCount")
      ...
      if (savedProductCount && savedScrollPosition) {
        ...
        if (targetCount > initialProducts.length) {
          ...
          try {
            ...
            console.error("[v0] Error restoring Nike products:", error)
          }
        }
        ...
        sessionStorage.removeItem("nikeScrollPosition")
        sessionStorage.removeItem("nikeLoadedCount")
      }
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      sessionStorage.setItem("nikeScrollPosition", window.scrollY.toString())
      sessionStorage.setItem("nikeLoadedCount", products.length.toString())
    }
  ...
  const loadMoreProducts = async () => {
    ...
    try {
      ...
      if (error) {
        console.error("[v0] Error loading more Nike products:", error)
        return
      }
      ...
      console.error("[v0] Exception loading more Nike products:", error)
    } finally {
      setIsLoading(false)
    }
  }
      ...
      <section className="w-full max-w-[1920px] mx-auto mb-16">
        <div className="grid lg:grid-cols-2">
          ...
          <div className="bg-[#000000] px-8 py-12 lg:px-16 lg:py-16 flex flex-col justify-center">
            <h1 className="text-white text-4xl lg:text-5xl font-bold tracking-tight uppercase mb-6">NIKE</h1>
            <p className="text-white/90 text-base lg:text-lg leading-relaxed max-w-xl">
              Since 1964, Nike has been at the forefront of athletic innovation, transforming performance footwear and sportswear into cultural icons. From the revolutionary Air technology to collaborations with world-class athletes, Nike has consistently pushed boundaries. Explore a legacy built on innovation, style, and the relentless pursuit of excellence—from the track to the streets.
            </p>
          </div>
