...
export function GivenchyClient({
  initialProducts,
  totalCount,
  initialLimit,
  currentCategory,
  ...
  const buildFilterUrl = (type: "category" | "sort", value: string) => {
    ...
    return `/brands/givenchy${params.toString() ? `?${params.toString()}` : ""}`
  }

  useEffect(() => {
    const restoreState = async () => {
      const savedScrollPosition = sessionStorage.getItem("givenchyScrollPosition")
      const savedProductCount = sessionStorage.getItem("givenchyLoadedCount")
      ...
      if (savedProductCount && savedScrollPosition) {
        ...
        if (targetCount > initialProducts.length) {
          ...
          try {
            ...
            console.error("[v0] Error restoring Givenchy products:", error)
          }
        }
        ...
        sessionStorage.removeItem("givenchyScrollPosition")
        sessionStorage.removeItem("givenchyLoadedCount")
      }
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      sessionStorage.setItem("givenchyScrollPosition", window.scrollY.toString())
      sessionStorage.setItem("givenchyLoadedCount", products.length.toString())
    }
  ...
  const loadMoreProducts = async () => {
    ...
    try {
      ...
      if (error) {
        console.error("[v0] Error loading more Givenchy products:", error)
        return
      }
      ...
      console.error("[v0] Exception loading more Givenchy products:", error)
    } finally {
      setIsLoading(false)
    }
  }
      ...
      <section className="w-full max-w-[1920px] mx-auto mb-16">
        <div className="grid lg:grid-cols-2">
          ...
          <div className="bg-[#000000] px-8 py-12 lg:px-16 lg:py-16 flex flex-col justify-center">
            <h1 className="text-white text-4xl lg:text-5xl font-bold tracking-tight uppercase mb-6">GIVENCHY</h1>
            <p className="text-white/90 text-base lg:text-lg leading-relaxed max-w-xl">
              Founded by Hubert de Givenchy in 1952, Givenchy embodies Parisian haute couture with rebellious elegance. From dressing Audrey Hepburn to defining contemporary luxury, the French fashion house seamlessly blends aristocratic sophistication with modern edge. Explore timeless ready-to-wear, accessories, and statement pieces that continue to shape the global fashion landscape.
            </p>
          </div>
