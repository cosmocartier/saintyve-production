...
interface FerragamoClientProps {
...
export function FerragamoClient({
  initialProducts,
  totalCount,
  initialLimit,
  currentCategory,
  ...
  const buildFilterUrl = (type: "category" | "sort", value: string) => {
    ...
    return `/brands/ferragamo${params.toString() ? `?${params.toString()}` : ""}`
  }

  useEffect(() => {
    const restoreState = async () => {
      const savedScrollPosition = sessionStorage.getItem("ferragamoScrollPosition")
      const savedProductCount = sessionStorage.getItem("ferragamoLoadedCount")
      ...
      if (savedProductCount && savedScrollPosition) {
        ...
        if (targetCount > initialProducts.length) {
          ...
          try {
            ...
            console.error("[v0] Error restoring Ferragamo products:", error)
          }
        }
        ...
        sessionStorage.removeItem("ferragamoScrollPosition")
        sessionStorage.removeItem("ferragamoLoadedCount")
      }
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      sessionStorage.setItem("ferragamoScrollPosition", window.scrollY.toString())
      sessionStorage.setItem("ferragamoLoadedCount", products.length.toString())
    }
  ...
  const loadMoreProducts = async () => {
    ...
    try {
      ...
      if (error) {
        console.error("[v0] Error loading more Ferragamo products:", error)
        return
      }
      ...
      console.error("[v0] Exception loading more Ferragamo products:", error)
    } finally {
      setIsLoading(false)
    }
  }
      ...
      <section className="w-full max-w-[1920px] mx-auto mb-16">
        <div className="grid lg:grid-cols-2">
          ...
          <div className="bg-[#000000] px-8 py-12 lg:px-16 lg:py-16 flex flex-col justify-center">
            <h1 className="text-white text-4xl lg:text-5xl font-bold tracking-tight uppercase mb-6">FERRAGAMO</h1>
            <p className="text-white/90 text-base lg:text-lg leading-relaxed max-w-xl">
              Founded in 1927 by Salvatore Ferragamo in Florence, Italy, this legendary house has epitomized Italian luxury craftsmanship for nearly a century. Renowned for its innovative approach to footwear design and impeccable quality, Ferragamo has dressed Hollywood icons and global tastemakers. From the iconic Vara pumps to sophisticated leather goods and refined ready-to-wear, each piece embodies timeless elegance and the spirit of "made in Italy" excellence.
            </p>
          </div>
