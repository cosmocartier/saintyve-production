...
interface AirJordanClientProps {
...
export function AirJordanClient({
  initialProducts,
  totalCount,
  initialLimit,
  currentCategory,
  ...
  const buildFilterUrl = (type: "category" | "sort", value: string) => {
    ...
    return `/brands/air-jordan${params.toString() ? `?${params.toString()}` : ""}`
  }

  useEffect(() => {
    const restoreState = async () => {
      const savedScrollPosition = sessionStorage.getItem("airJordanScrollPosition")
      const savedProductCount = sessionStorage.getItem("airJordanLoadedCount")
      ...
      if (savedProductCount && savedScrollPosition) {
        ...
        if (targetCount > initialProducts.length) {
          ...
          try {
            ...
            console.error("[v0] Error restoring Jordan products:", error)
          }
        }
        ...
        sessionStorage.removeItem("airJordanScrollPosition")
        sessionStorage.removeItem("airJordanLoadedCount")
      }
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      sessionStorage.setItem("airJordanScrollPosition", window.scrollY.toString())
      sessionStorage.setItem("airJordanLoadedCount", products.length.toString())
    }
  ...
  const loadMoreProducts = async () => {
    ...
    try {
      ...
      if (error) {
        console.error("[v0] Error loading more Jordan products:", error)
        return
      }
      ...
      console.error("[v0] Exception loading more Jordan products:", error)
    } finally {
      setIsLoading(false)
    }
  }
      ...
      <section className="w-full max-w-[1920px] mx-auto mb-16">
        <div className="grid lg:grid-cols-2">
          ...
          <div className="bg-[#000000] px-8 py-12 lg:px-16 lg:py-16 flex flex-col justify-center">
            <h1 className="text-white text-4xl lg:text-5xl font-bold tracking-tight uppercase mb-6">AIR JORDAN</h1>
            <p className="text-white/90 text-base lg:text-lg leading-relaxed max-w-xl">
              Born from the legendary partnership between Nike and Michael Jordan in 1984, Air Jordan has become more
              than just footwear—it's a cultural phenomenon. From the groundbreaking Air Jordan 1 to the latest
              innovations, each silhouette tells a story of excellence, determination, and revolutionary design. Explore
              iconic sneakers and apparel that continue to define basketball culture and streetwear worldwide.
            </p>
          </div>
