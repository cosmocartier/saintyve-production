...
export function GoyardClient({
  ...
  const buildFilterUrl = (type: "category" | "sort", value: string) => {
    ...
    return `/brands/goyard${params.toString() ? `?${params.toString()}` : ""}`
  }

  useEffect(() => {
    const restoreState = async () => {
      const savedScrollPosition = sessionStorage.getItem("goyardScrollPosition")
      const savedProductCount = sessionStorage.getItem("goyardLoadedCount")
      ...
      if (savedProductCount && savedScrollPosition) {
        ...
        if (targetCount > initialProducts.length) {
          ...
          try {
            ...
            console.error("[v0] Error restoring Goyard products:", error)
          }
        }
        ...
        sessionStorage.removeItem("goyardScrollPosition")
        sessionStorage.removeItem("goyardLoadedCount")
      }
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      sessionStorage.setItem("goyardScrollPosition", window.scrollY.toString())
      sessionStorage.setItem("goyardLoadedCount", products.length.toString())
    }
  ...
  const loadMoreProducts = async () => {
    ...
    try {
      ...
      if (error) {
        console.error("[v0] Error loading more Goyard products:", error)
        return
      }
      ...
      console.error("[v0] Exception loading more Goyard products:", error)
    } finally {
      setIsLoading(false)
    }
  }
      ...
      <section className="w-full max-w-[1920px] mx-auto mb-16">
        <div className="grid lg:grid-cols-2">
          ...
          <div className="bg-[#000000] px-8 py-12 lg:px-16 lg:py-16 flex flex-col justify-center">
            <h1 className="text-white text-4xl lg:text-5xl font-bold tracking-tight uppercase mb-6">GOYARD</h1>
            <p className="text-white/90 text-base lg:text-lg leading-relaxed max-w-xl">
              Established in 1853, Goyard stands as one of the oldest luxury French trunk and leather goods makers. 
              Known for its iconic Goyardine canvas with the distinctive chevron pattern, each piece is hand-painted 
              and crafted with meticulous attention to detail. From the legendary Saint Louis tote to elegant trunks 
              and accessories, Goyard represents the pinnacle of French craftsmanship and timeless luxury that has 
              remained exclusive and coveted for over 170 years.
            </p>
          </div>
