...
export function DiorClient({
  ...
  const buildFilterUrl = (type: "category" | "sort", value: string) => {
    ...
    return `/brands/dior${params.toString() ? `?${params.toString()}` : ""}`
  }

  useEffect(() => {
    const restoreState = async () => {
      const savedScrollPosition = sessionStorage.getItem("diorScrollPosition")
      const savedProductCount = sessionStorage.getItem("diorLoadedCount")
      ...
      if (savedProductCount && savedScrollPosition) {
        ...
        if (targetCount > initialProducts.length) {
          ...
          try {
            ...
            console.error("[v0] Error restoring Dior products:", error)
          }
        }
        ...
        sessionStorage.removeItem("diorScrollPosition")
        sessionStorage.removeItem("diorLoadedCount")
      }
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      sessionStorage.setItem("diorScrollPosition", window.scrollY.toString())
      sessionStorage.setItem("diorLoadedCount", products.length.toString())
    }
  ...
  const loadMoreProducts = async () => {
    ...
    try {
      ...
      if (error) {
        console.error("[v0] Error loading more Dior products:", error)
        return
      }
      ...
      console.error("[v0] Exception loading more Dior products:", error)
    } finally {
      setIsLoading(false)
    }
  }
      ...
      <section className="w-full max-w-[1920px] mx-auto mb-16">
        <div className="grid lg:grid-cols-2">
          ...
          <div className="bg-[#000000] px-8 py-12 lg:px-16 lg:py-16 flex flex-col justify-center">
            <h1 className="text-white text-4xl lg:text-5xl font-bold tracking-tight uppercase mb-6">DIOR</h1>
            <p className="text-white/90 text-base lg:text-lg leading-relaxed max-w-xl">
              Since 1946, the House of Dior has epitomized French luxury and elegance. Founded by Christian Dior, the
              maison revolutionized post-war fashion with the iconic "New Look" silhouette. Today, Dior continues to
              blend timeless sophistication with contemporary innovation across haute couture, ready-to-wear, and
              accessories. Discover a legacy of craftsmanship and refined style that has shaped modern luxury fashion.
            </p>
          </div>
