...
export function CelineClient({
  ...
  const buildFilterUrl = (type: "category" | "sort", value: string) => {
    ...
    return `/brands/celine${params.toString() ? `?${params.toString()}` : ""}`
  }

  useEffect(() => {
    const restoreState = async () => {
      const savedScrollPosition = sessionStorage.getItem("celineScrollPosition")
      const savedProductCount = sessionStorage.getItem("celineLoadedCount")
      ...
      if (savedProductCount && savedScrollPosition) {
        ...
        if (targetCount > initialProducts.length) {
          ...
          try {
            ...
            console.error("[v0] Error restoring Celine products:", error)
          }
        }
        ...
        sessionStorage.removeItem("celineScrollPosition")
        sessionStorage.removeItem("celineLoadedCount")
      }
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      sessionStorage.setItem("celineScrollPosition", window.scrollY.toString())
      sessionStorage.setItem("celineLoadedCount", products.length.toString())
    }
  ...
  const loadMoreProducts = async () => {
    ...
    try {
      ...
      if (error) {
        console.error("[v0] Error loading more Celine products:", error)
        return
      }
      ...
      console.error("[v0] Exception loading more Celine products:", error)
    } finally {
      setIsLoading(false)
    }
  }
      ...
      <section className="w-full max-w-[1920px] mx-auto mb-16">
        <div className="grid lg:grid-cols-2">
          ...
          <div className="bg-[#000000] px-8 py-12 lg:px-16 lg:py-16 flex flex-col justify-center">
            <h1 className="text-white text-4xl lg:text-5xl font-bold tracking-tight uppercase mb-6">CELINE</h1>
            <p className="text-white/90 text-base lg:text-lg leading-relaxed max-w-xl">
              Founded in 1945 by Céline Vipiana, Celine has evolved into one of the most influential luxury houses in fashion. Known for its minimalist aesthetic and architectural precision, the Parisian maison embodies effortless sophistication. From the iconic Triomphe monogram to contemporary leather goods and ready-to-wear, Celine represents timeless elegance with a modern sensibility.
            </p>
          </div>
