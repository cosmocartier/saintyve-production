...
export function PradaClient({
  ...
  const buildFilterUrl = (type: "category" | "sort", value: string) => {
    ...
    return `/brands/prada${params.toString() ? `?${params.toString()}` : ""}`
  }

  useEffect(() => {
    const restoreState = async () => {
      const savedScrollPosition = sessionStorage.getItem("pradaScrollPosition")
      const savedProductCount = sessionStorage.getItem("pradaLoadedCount")
      ...
      if (savedProductCount && savedScrollPosition) {
        ...
        if (targetCount > initialProducts.length) {
          ...
          try {
            ...
            console.error("[v0] Error restoring Prada products:", error)
          }
        }
        ...
        sessionStorage.removeItem("pradaScrollPosition")
        sessionStorage.removeItem("pradaLoadedCount")
      }
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      sessionStorage.setItem("pradaScrollPosition", window.scrollY.toString())
      sessionStorage.setItem("pradaLoadedCount", products.length.toString())
    }
  ...
  const loadMoreProducts = async () => {
    ...
    try {
      ...
      if (error) {
        console.error("[v0] Error loading more Prada products:", error)
        return
      }
      ...
      console.error("[v0] Exception loading more Prada products:", error)
    } finally {
      setIsLoading(false)
    }
  }
      ...
      <section className="w-full max-w-[1920px] mx-auto mb-16">
        <div className="grid lg:grid-cols-2">
          ...
          <div className="bg-[#000000] px-8 py-12 lg:px-16 lg:py-16 flex flex-col justify-center">
            <h1 className="text-white text-4xl lg:text-5xl font-bold tracking-tight uppercase mb-6">PRADA</h1>
            <p className="text-white/90 text-base lg:text-lg leading-relaxed max-w-xl">
              Founded in Milan in 1913, Prada has defined Italian luxury for over a century. From the iconic Saffiano 
              leather to groundbreaking fashion that blends tradition with avant-garde innovation, Prada represents 
              sophisticated craftsmanship and intellectual elegance. Explore a collection that continues to push 
              boundaries while honoring timeless quality and refined aesthetics.
            </p>
          </div>
