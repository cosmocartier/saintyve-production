...
interface BottegaVenetaClientProps {
...
export function BottegaVenetaClient({
  initialProducts,
  totalCount,
  initialLimit,
  currentCategory,
  ...
  const buildFilterUrl = (type: "category" | "sort", value: string) => {
    ...
    return `/brands/bottega-veneta${params.toString() ? `?${params.toString()}` : ""}`
  }

  useEffect(() => {
    const restoreState = async () => {
      const savedScrollPosition = sessionStorage.getItem("bottegaVenetaScrollPosition")
      const savedProductCount = sessionStorage.getItem("bottegaVenetaLoadedCount")
      ...
      if (savedProductCount && savedScrollPosition) {
        ...
        if (targetCount > initialProducts.length) {
          ...
          try {
            ...
            console.error("[v0] Error restoring Bottega Veneta products:", error)
          }
        }
        ...
        sessionStorage.removeItem("bottegaVenetaScrollPosition")
        sessionStorage.removeItem("bottegaVenetaLoadedCount")
      }
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      sessionStorage.setItem("bottegaVenetaScrollPosition", window.scrollY.toString())
      sessionStorage.setItem("bottegaVenetaLoadedCount", products.length.toString())
    }
  ...
  const loadMoreProducts = async () => {
    ...
    try {
      ...
      if (error) {
        console.error("[v0] Error loading more Bottega Veneta products:", error)
        return
      }
      ...
      console.error("[v0] Exception loading more Bottega Veneta products:", error)
    } finally {
      setIsLoading(false)
    }
  }
    ...
    <>
      <section className="w-full max-w-[1920px] mx-auto mb-16">
        <div className="grid lg:grid-cols-2">
          ...
          <div className="bg-[#2C3E2E] px-8 py-12 lg:px-16 lg:py-16 flex flex-col justify-center">
            <h1 className="text-white text-4xl lg:text-5xl font-bold tracking-tight uppercase mb-6">BOTTEGA VENETA</h1>
            <p className="text-white/90 text-base lg:text-lg leading-relaxed max-w-xl">
              Founded in Vicenza in 1966, Bottega Veneta is synonymous with Italian craftsmanship and timeless luxury. The house's signature intrecciato woven leather technique represents understated elegance and meticulous artisanship. Today, the brand continues to define contemporary luxury with innovative designs that honor its heritage while pushing creative boundaries. Discover pieces that embody quiet confidence and exceptional quality.
            </p>
          </div>
