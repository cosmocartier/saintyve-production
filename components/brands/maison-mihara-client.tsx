...
interface MaisonMiharaClientProps {
...
export function MaisonMiharaClient({
  initialProducts,
  totalCount,
  initialLimit,
  currentCategory,
  ...
  const buildFilterUrl = (type: "category" | "sort", value: string) => {
    ...
    return `/brands/maison-mihara${params.toString() ? `?${params.toString()}` : ""}`
  }

  useEffect(() => {
    const restoreState = async () => {
      const savedScrollPosition = sessionStorage.getItem("maisonMiharaScrollPosition")
      const savedProductCount = sessionStorage.getItem("maisonMiharaLoadedCount")
      ...
      if (savedProductCount && savedScrollPosition) {
        ...
        if (targetCount > initialProducts.length) {
          ...
          try {
            ...
            console.error("[v0] Error restoring Maison Mihara products:", error)
          }
        }
        ...
        sessionStorage.removeItem("maisonMiharaScrollPosition")
        sessionStorage.removeItem("maisonMiharaLoadedCount")
      }
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      sessionStorage.setItem("maisonMiharaScrollPosition", window.scrollY.toString())
      sessionStorage.setItem("maisonMiharaLoadedCount", products.length.toString())
    }
  ...
  const loadMoreProducts = async () => {
    ...
    try {
      ...
      if (error) {
        console.error("[v0] Error loading more Maison Mihara products:", error)
        return
      }
      ...
      console.error("[v0] Exception loading more Maison Mihara products:", error)
    } finally {
      setIsLoading(false)
    }
  }
      ...
      <section className="w-full max-w-[1920px] mx-auto mb-16">
        <div className="grid lg:grid-cols-2">
          ...
          <div className="bg-[#000000] px-8 py-12 lg:px-16 lg:py-16 flex flex-col justify-center">
            <h1 className="text-white text-4xl lg:text-5xl font-bold tracking-tight uppercase mb-6">MAISON MIHARA YASUHIRO</h1>
            <p className="text-white/90 text-base lg:text-lg leading-relaxed max-w-xl">
              Founded by Japanese designer Mihara Yasuhiro in 1997, Maison Mihara Yasuhiro redefines contemporary footwear with avant-garde aesthetics and deconstructed designs. Known for worn-in textures, distressed finishes, and boundary-pushing silhouettes, the brand fuses high fashion with streetwear sensibility. Each piece embodies innovation, craftsmanship, and the spirit of artistic rebellion.
            </p>
          </div>
