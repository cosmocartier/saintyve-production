...
export function CartierClient({
  ...
  const buildFilterUrl = (type: "category" | "sort", value: string) => {
    ...
    return `/brands/cartier${params.toString() ? `?${params.toString()}` : ""}`
  }

  useEffect(() => {
    const restoreState = async () => {
      const savedScrollPosition = sessionStorage.getItem("cartierScrollPosition")
      const savedProductCount = sessionStorage.getItem("cartierLoadedCount")
      ...
      if (savedProductCount && savedScrollPosition) {
        ...
        if (targetCount > initialProducts.length) {
          ...
          try {
            ...
            console.error("[v0] Error restoring Cartier products:", error)
          }
        }
        ...
        sessionStorage.removeItem("cartierScrollPosition")
        sessionStorage.removeItem("cartierLoadedCount")
      }
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      sessionStorage.setItem("cartierScrollPosition", window.scrollY.toString())
      sessionStorage.setItem("cartierLoadedCount", products.length.toString())
    }
  ...
  const loadMoreProducts = async () => {
    ...
    try {
      ...
      if (error) {
        console.error("[v0] Error loading more Cartier products:", error)
        return
      }
      ...
      console.error("[v0] Exception loading more Cartier products:", error)
    } finally {
      setIsLoading(false)
    }
  }
      ...
      <section className="w-full max-w-[1920px] mx-auto mb-16">
        <div className="grid lg:grid-cols-2">
          ...
          <div className="bg-[#000000] px-8 py-12 lg:px-16 lg:py-16 flex flex-col justify-center">
            <h1 className="text-white text-4xl lg:text-5xl font-bold tracking-tight uppercase mb-6">CARTIER</h1>
            <p className="text-white/90 text-base lg:text-lg leading-relaxed max-w-xl">
              Founded in Paris in 1847, Cartier has become synonymous with luxury, elegance, and timeless sophistication. 
              From the legendary Love bracelet to the iconic Tank watch, each Cartier creation represents the pinnacle of 
              French craftsmanship and design excellence. Explore a heritage collection that has adorned royalty, celebrities, 
              and connoisseurs of fine jewelry for over 175 years.
            </p>
          </div>
