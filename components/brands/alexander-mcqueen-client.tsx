...
interface AlexanderMcQueenClientProps {
...
export function AlexanderMcQueenClient({
  initialProducts,
  totalCount,
  initialLimit,
  currentCategory,
  ...
  const buildFilterUrl = (type: "category" | "sort", value: string) => {
    ...
    return `/brands/alexander-mcqueen${params.toString() ? `?${params.toString()}` : ""}`
  }

  useEffect(() => {
    const restoreState = async () => {
      const savedScrollPosition = sessionStorage.getItem("alexanderMcQueenScrollPosition")
      const savedProductCount = sessionStorage.getItem("alexanderMcQueenLoadedCount")
      ...
      if (savedProductCount && savedScrollPosition) {
        ...
        if (targetCount > initialProducts.length) {
          ...
          try {
            ...
            console.error("[v0] Error restoring Alexander McQueen products:", error)
          }
        }
        ...
        sessionStorage.removeItem("alexanderMcQueenScrollPosition")
        sessionStorage.removeItem("alexanderMcQueenLoadedCount")
      }
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      sessionStorage.setItem("alexanderMcQueenScrollPosition", window.scrollY.toString())
      sessionStorage.setItem("alexanderMcQueenLoadedCount", products.length.toString())
    }
  ...
  const loadMoreProducts = async () => {
    ...
    try {
      ...
      if (error) {
        console.error("[v0] Error loading more Alexander McQueen products:", error)
        return
      }
      ...
      console.error("[v0] Exception loading more Alexander McQueen products:", error)
    } finally {
      setIsLoading(false)
    }
  }
      ...
      <section className="w-full max-w-[1920px] mx-auto mb-16">
        <div className="grid lg:grid-cols-2">
          ...
          <div className="bg-[#000000] px-8 py-12 lg:px-16 lg:py-16 flex flex-col justify-center">
            <h1 className="text-white text-4xl lg:text-5xl font-bold tracking-tight uppercase mb-6">ALEXANDER MCQUEEN</h1>
            <p className="text-white/90 text-base lg:text-lg leading-relaxed max-w-xl">
              Renowned for dark romanticism and avant-garde tailoring, Alexander McQueen represents British luxury at its most daring. From the iconic skull motif to oversized sneakers and dramatic silhouettes, the brand pushes creative boundaries while maintaining exceptional craftsmanship. Each piece embodies the late designer's fearless vision—a fusion of rebellion, artistry, and uncompromising elegance that continues to redefine contemporary fashion.
            </p>
          </div>
