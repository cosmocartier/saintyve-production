...
interface ValentinoGaravaniClientProps {
...
export function ValentinoGaravaniClient({
  initialProducts,
  totalCount,
  initialLimit,
  currentCategory,
  ...
  const buildFilterUrl = (type: "category" | "sort", value: string) => {
    ...
    return `/brands/valentino-garavani${params.toString() ? `?${params.toString()}` : ""}`
  }

  useEffect(() => {
    const restoreState = async () => {
      const savedScrollPosition = sessionStorage.getItem("valentinoGaravaniScrollPosition")
      const savedProductCount = sessionStorage.getItem("valentinoGaravaniLoadedCount")
      ...
      if (savedProductCount && savedScrollPosition) {
        ...
        if (targetCount > initialProducts.length) {
          ...
          try {
            ...
            console.error("[v0] Error restoring Valentino Garavani products:", error)
          }
        }
        ...
        sessionStorage.removeItem("valentinoGaravaniScrollPosition")
        sessionStorage.removeItem("valentinoGaravaniLoadedCount")
      }
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      sessionStorage.setItem("valentinoGaravaniScrollPosition", window.scrollY.toString())
      sessionStorage.setItem("valentinoGaravaniLoadedCount", products.length.toString())
    }
  ...
  const loadMoreProducts = async () => {
    ...
    try {
      ...
      if (error) {
        console.error("[v0] Error loading more Valentino Garavani products:", error)
        return
      }
      ...
      console.error("[v0] Exception loading more Valentino Garavani products:", error)
    } finally {
      setIsLoading(false)
    }
  }
      ...
      <section className="w-full max-w-[1920px] mx-auto mb-16">
        <div className="grid lg:grid-cols-2">
          ...
          <div className="bg-[#000000] px-8 py-12 lg:px-16 lg:py-16 flex flex-col justify-center">
            <h1 className="text-white text-4xl lg:text-5xl font-bold tracking-tight uppercase mb-6">VALENTINO GARAVANI</h1>
            <p className="text-white/90 text-base lg:text-lg leading-relaxed max-w-xl">
              Founded in 1960 by Valentino Garavani in Rome, Valentino represents the pinnacle of Italian haute couture and luxury craftsmanship. From the iconic Rockstud collection to timeless red carpet gowns, Valentino embodies romantic elegance, bold innovation, and meticulous attention to detail. Discover a heritage of sophistication that has dressed royalty, celebrities, and fashion icons for over six decades.
            </p>
          </div>
