...
export function YSLClient({
  ...
  const buildFilterUrl = (type: "category" | "sort", value: string) => {
    ...
    return `/brands/ysl${params.toString() ? `?${params.toString()}` : ""}`
  }

  useEffect(() => {
    const restoreState = async () => {
      const savedScrollPosition = sessionStorage.getItem("yslScrollPosition")
      const savedProductCount = sessionStorage.getItem("yslLoadedCount")
      ...
      if (savedProductCount && savedScrollPosition) {
        ...
        if (targetCount > initialProducts.length) {
          ...
          try {
            ...
            console.error("[v0] Error restoring YSL products:", error)
          }
        }
        ...
        sessionStorage.removeItem("yslLoadedCount")
      }
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      ...
      sessionStorage.setItem("yslLoadedCount", products.length.toString())
    }
  ...
  const loadMoreProducts = async () => {
    ...
    try {
      ...
      if (error) {
        console.error("[v0] Error loading more YSL products:", error)
        return
      }
      ...
      console.error("[v0] Exception loading more YSL products:", error)
    } finally {
      setIsLoading(false)
    }
  }
      ...
      <section className="w-full max-w-[1920px] mx-auto mb-16">
        <div className="grid lg:grid-cols-2">
          ...
          <div className="bg-[#000000] px-8 py-12 lg:px-16 lg:py-16 flex flex-col justify-center">
            <h1 className="text-white text-4xl lg:text-5xl font-bold tracking-tight uppercase mb-6">YSL</h1>
            <p className="text-white/90 text-base lg:text-lg leading-relaxed max-w-xl">
              Founded by Yves Saint Laurent in 1961, YSL revolutionized fashion with its bold vision and timeless elegance. 
              From the iconic Le Smoking tuxedo to contemporary ready-to-wear and accessories, the house embodies Parisian 
              sophistication and daring innovation. Explore a legacy of haute couture excellence, where classic craftsmanship 
              meets modern luxury in every meticulously designed piece.
            </p>
          </div>
