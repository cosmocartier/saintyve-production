...
interface BalenciagaClientProps {
...
export function BalenciagaClient({
  initialProducts,
  totalCount,
  initialLimit,
  currentCategory,
  ...
  const buildFilterUrl = (type: "category" | "sort", value: string) => {
    ...
    return `/brands/balenciaga${params.toString() ? `?${params.toString()}` : ""}`
  }

  useEffect(() => {
    const restoreState = async () => {
      const savedScrollPosition = sessionStorage.getItem("balenciagaScrollPosition")
      const savedProductCount = sessionStorage.getItem("balenciagaLoadedCount")
      ...
      if (savedProductCount && savedScrollPosition) {
        ...
        if (targetCount > initialProducts.length) {
          ...
          try {
            ...
            console.error("[v0] Error restoring Balenciaga products:", error)
          }
        }
        ...
        sessionStorage.removeItem("balenciagaScrollPosition")
        sessionStorage.removeItem("balenciagaLoadedCount")
      }
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      sessionStorage.setItem("balenciagaScrollPosition", window.scrollY.toString())
      sessionStorage.setItem("balenciagaLoadedCount", products.length.toString())
    }
  ...
  const loadMoreProducts = async () => {
    ...
    try {
      ...
      if (error) {
        console.error("[v0] Error loading more Balenciaga products:", error)
        return
      }
      ...
      console.error("[v0] Exception loading more Balenciaga products:", error)
    } finally {
      setIsLoading(false)
    }
  }
      ...
      <section className="w-full max-w-[1920px] mx-auto mb-16">
        <div className="grid lg:grid-cols-2">
          ...
          <div className="bg-[#000000] px-8 py-12 lg:px-16 lg:py-16 flex flex-col justify-center">
            <h1 className="text-white text-4xl lg:text-5xl font-bold tracking-tight uppercase mb-6">BALENCIAGA</h1>
            <p className="text-white/90 text-base lg:text-lg leading-relaxed max-w-xl">
              Founded by Cristóbal Balenciaga in 1919, this legendary house has redefined luxury fashion with its
              avant-garde approach and boundary-pushing designs. Under the creative direction of Demna, Balenciaga
              continues to challenge conventions with oversized silhouettes, deconstructed forms, and provocative
              statements. From the iconic Triple S to bold ready-to-wear, explore pieces that blur the lines between
              high fashion and streetwear culture.
            </p>
          </div>
