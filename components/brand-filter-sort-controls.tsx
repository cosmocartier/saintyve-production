...
interface BrandFilterSortControlsProps {
  // Category filter state (for brand pages, this is the product category)
  categories: string[]
  ...
  // URL builders
  buildFilterUrl: (type: "category" | "sort", value: string) => string

  // Total count
  totalCount: number
}

export function BrandFilterSortControls({
  categories,
  currentCategory,
    ...
    <div className="px-4 mb-0">
      <div className="flex items-center justify-between">
        ...
        <div className="relative" ref={categoryRef}>
          ...
          >
            <span>{currentCategory || "All Items"}</span>
            <ChevronDown
                ...
                <div className="py-4 px-4">
                  <Link
                    ...
                    className={`flex items-center justify-between px-3 py-2 text-[14px] leading-[1.4] tracking-[0.01em] rounded-md transition-all duration-[160ms] ${
                      !currentCategory ? "font-medium text-[#111]" : "font-normal text-[#7A7A7A] hover:bg-black/[0.015]"
                    }`}
                  >
                    <span>All Items</span>
                    {!currentCategory && <Check className="w-3.5 h-3.5 text-[#111] opacity-70" />}
                  </Link>
                  ...
                  {categories.map((category) => (
                    ...
                    >
                      <span>{category}</span>
                      {currentCategory === category && <Check className="w-3.5 h-3.5 text-[#111] opacity-70" />}
                    </Link>
                  ))}
                </div>
              </div>
