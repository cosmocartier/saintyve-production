...
const navigationSections = [
  {
    label: "WORKSPACE",
    items: [{ name: "Dashboard", href: "/admin/supplier", icon: LayoutDashboard }],
  },
  {
    label: "FULFILLMENT",
    items: [
      { name: "Orders", href: "/admin/supplier/orders", icon: ShoppingCart },
      { name: "Tracking", href: "/admin/supplier/tracking", icon: Truck },
      { name: "Investigations", href: "/admin/supplier/investigations", icon: AlertTriangle },
      { name: "Invoices", href: "/admin/supplier/invoices", icon: FileText },
    ],
  },
]
...
export function SupplierSidebar({ userRole, userEmail }: SupplierSidebarProps) {
    ...
    <div className="w-72 bg-zinc-950 text-white sticky top-0 h-screen flex flex-col overflow-y-auto border-r border-white/10">
      <div className="px-6 pt-8 pb-6 border-b border-white/10">
        {isAdmin ? (
          <DropdownMenu>
            <DropdownMenuTrigger className="w-full text-left hover:opacity-80 transition-opacity">
              <div className="flex items-center justify-between">
                <div>
                  <h1 className="text-2xl tracking-[0.25em] uppercase mb-1 font-medium">SUPPLIER</h1>
                  <p className="text-xs text-zinc-400 tracking-wide">Supplier Dashboard</p>
                </div>
          ...
          <div>
            <h1 className="text-2xl font-medium tracking-[0.25em] uppercase mb-1">SUPPLIER</h1>
            <p className="text-xs text-zinc-400 tracking-wide">Supplier Dashboard</p>
          </div>
        )}
      </div>
      ...
      <div className="px-4 pb-6 pt-4 border-t border-white/10 space-y-4">
        <div className="flex items-center gap-3 px-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-white truncate">{userEmail || "Supplier"}</p>
            <p className="text-xs text-zinc-500 truncate">Portal Access</p>
          </div>
        </div>
