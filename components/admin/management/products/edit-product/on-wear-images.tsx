...
interface OnWearImagesSectionProps {
  productId: string
  initialImages: OnWearImage[]
  onUpdate: () => void
}

export function OnWearImagesSection({ productId, initialImages, onUpdate }: OnWearImagesSectionProps) {
  const [images, setImages] = useState<OnWearImage[]>(initialImages)
  ...
  const remainingSlots = MAX_ON_WEAR_IMAGES - images.length
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files.length > remainingSlots) {
      alert(
        `On-wear images are limited to ${MAX_ON_WEAR_IMAGES}. You can add ${remainingSlots} more.`,
      )
    ...
    try {
      ...
      const response = await fetch(`/api/admin/products/${productId}/on-wear-images`, {
        method: "POST",
        body: formData,
      })
  ...
  const handleDeleteImage = async (imageId: string) => {
    ...
    try {
      const response = await fetch(`/api/admin/products/${productId}/on-wear-images/${imageId}`, {
        method: "DELETE",
      })
  ...
  const handleDragEnd = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/products/${productId}/on-wear-images/reorder`, {
        method: "PATCH",
  ...
  const handleUpdateImageAlt = async (imageId: string) => {
    try {
      await fetch(`/api/admin/products/${productId}/on-wear-images/${imageId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
      ...
      console.error("[On-Wear Images] Alt text update error:", error)
  ...
  return (
    <div className="border border-zinc-200 rounded-md p-6 bg-white">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-sm font-medium tracking-wide uppercase text-zinc-900">
          ON-WEAR IMAGES <span className="text-zinc-400 normal-case tracking-normal">({images.length}/{MAX_ON_WEAR_IMAGES})</span>
        </h2>
        <button
          type="button"
          onClick={() => fileInputRef.current?.click()}
          disabled={uploading || isFull}
          className="inline-flex items-center gap-2 border border-black rounded-md px-3 py-2 text-xs font-medium uppercase tracking-wide transition-colors hover:bg-black hover:text-white disabled:opacity-50 disabled:hover:bg-transparent disabled:hover:text-black"
        >
          <Upload size={14} />
          {uploading ? "Uploading..." : isFull ? "Limit Reached" : "Add Images"}
        </button>
        <input
          ref={fileInputRef}
          type="file"
      ...
      {images.length === 0 ? (
        <div className="border-2 border-dashed border-zinc-300 rounded-md py-8 text-center">
          ...
          <p className="text-xs font-medium text-zinc-600 uppercase tracking-wide">
            No on-wear images yet. Add up to {MAX_ON_WEAR_IMAGES} styled shots to show the on-wear mosaic on the product page.
          </p>
        </div>
      ) : (
                ...
                >
                  <div className="aspect-[4/5] relative">
                    <Image
                      ...
                      alt={image.alt_text || `On-wear image ${index + 1}`}
                      fill
      ...
      <p className="mt-3 text-xs font-medium text-zinc-500">
        Drag and drop images to reorder. These editorial &quot;on-wear&quot; shots appear only on the product page in a
        dedicated mosaic (max {MAX_ON_WEAR_IMAGES}). The first image fills the large top-left tile, image five fills the
        large bottom-right tile. If none are added, no on-wear mosaic is shown.
      </p>
    </div>
  )
}
