...
interface CategoryImagesSectionProps {
  productId: string
  initialImages: CategoryImage[]
  onUpdate: () => void
}

export function CategoryImagesSection({ productId, initialImages, onUpdate }: CategoryImagesSectionProps) {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/products/${productId}/category-images`, {
        method: "POST",
        body: formData,
      })
  ...
  const handleDeleteImage = async (imageId: string) => {
    ...
    try {
      const response = await fetch(`/api/admin/products/${productId}/category-images/${imageId}`, {
        method: "DELETE",
      })
      ...
      console.error("[Category Images] Delete error:", error)
  ...
  const handleDragEnd = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/products/${productId}/category-images/reorder`, {
        method: "PATCH",
      ...
      console.error("[Category Images] Reorder error:", error)
  ...
  const handleUpdateImageAlt = async (imageId: string) => {
    try {
      await fetch(`/api/admin/products/${productId}/category-images/${imageId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
      ...
      console.error("[Category Images] Alt text update error:", error)
  ...
  return (
    <div className="border border-zinc-200 rounded-md p-6 bg-white">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-sm font-medium tracking-wide uppercase text-zinc-900">CATEGORY / BRAND PAGE IMAGES</h2>
        <button
          type="button"
      ...
      {images.length === 0 ? (
        <div className="border-2 border-dashed border-zinc-300 rounded-md py-8 text-center">
          ...
          <p className="text-xs font-medium text-zinc-600 uppercase tracking-wide">
            No category images yet. Category and Brand pages will fall back to the default Cloudflare images above.
          </p>
        </div>
      ) : (
      ...
      <p className="mt-3 text-xs font-medium text-zinc-500">
        Drag and drop images to reorder. These images are used only on Category and Brand listing pages, in place of
        the default Cloudflare images above. If none are added, the listing pages fall back to the default images.
      </p>
    </div>
  )
}
