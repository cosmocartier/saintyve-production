...
interface MensAccessoriesClientProps {
...
export function MensAccessoriesClient({
  ...
  useEffect(() => {
    const restoreState = async () => {
      const savedScrollPosition = sessionStorage.getItem("mensAccessoriesPageScrollPosition")
      const savedProductCount = sessionStorage.getItem("mensAccessoriesPageLoadedCount")
      ...
      if (savedProductCount && savedScrollPosition) {
        ...
        if (targetCount > initialProducts.length) {
          ...
          try {
            ...
            if (!error && productCategories && productCategories.length > 0) {
              const restoredProducts = productCategories.map((pc: any) => ({ ...pc.products, sort_order: pc.sort_order }))
              const cfProductIds = restoredProducts.filter((p: any) => p.use_cloudflare_images).map((p: any) => p.id)
              const cfImagesMap = new Map<string, any[]>()
              if (cfProductIds.length > 0) {
                const { data: cfImages } = await supabase
                  .from("product_images_cf").select("*").in("product_id", cfProductIds).order("sort_order", { ascending: true })
                if (cfImages) {
                  cfImages.forEach((img: any) => {
                    if (!cfImagesMap.has(img.product_id)) cfImagesMap.set(img.product_id, [])
                    cfImagesMap.get(img.product_id)!.push({ id: img.id, url: buildCfUrl(img.cf_image_id, "grid"), alt_text: img.alt_text, display_order: img.sort_order, color_name: img.color_name, color_hex: img.color_hex })
                  })
                }
              }
              const optimizedProducts = restoredProducts.map((product: any) => {
                ...
                return { ...product, product_images: Array.from(imagesByColor.values()), has_multiple_images: images.length > imagesByColor.size }
              })
              setProducts(optimizedProducts)
              setCurrentOffset(productCategories.length)
            }
          } catch (err) {
            console.error("[v0] Error restoring products state:", err)
          }
        }
        ...
        sessionStorage.removeItem("mensAccessoriesPageScrollPosition")
        sessionStorage.removeItem("mensAccessoriesPageLoadedCount")
      }
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      sessionStorage.setItem("mensAccessoriesPageScrollPosition", window.scrollY.toString())
      sessionStorage.setItem("mensAccessoriesPageLoadedCount", products.length.toString())
    }
  ...
  const buildFilterUrl = () => {
    ...
    return `/mens/accessories${params.toString() ? `?${params.toString()}` : ""}`
  }

  const applyFilters = () => { window.location.href = buildFilterUrl() }
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting && !isLoading && hasMore) loadMoreProducts()
      },
      { root: null, rootMargin: "400px", threshold: 0 },
    )
    observer.observe(loadMoreTriggerRef.current)
    return () => observer.disconnect()
  }, [isLoading, hasMore, isRestoring, currentOffset])
    ...
    try {
      ...
      if (sortValue === "recommended") {
        const { products: smartProducts, error } = await getSmartSortedProducts({
          categoryId, offset: currentOffset, limit: batchSize,
          brands: selectedBrands.length > 0 ? selectedBrands : undefined,
          subcategories: selectedSubcategories.length > 0 ? selectedSubcategories : undefined,
          colors: selectedColors.length > 0 ? selectedColors : undefined,
          styles: selectedStyleTypes.length > 0 ? selectedStyleTypes : undefined,
        ...
        if (smartProducts && smartProducts.length > 0) {
          ...
          const unique = smartProducts.filter((p) => !existingIds.has(p.id))
          if (unique.length > 0) { setProducts((prev) => [...prev, ...unique]); setCurrentOffset((prev) => prev + unique.length) }
        }
      } else {
        const supabase = createBrowserClient()
        let q = supabase
          .from("product_categories")
          .select(`product_id, sort_order, products!inner (*, product_variants (id, size, stock_quantity), product_images (id, url, alt_text, display_order, color_name, color_hex))`)
          .eq("category_id", categoryId).eq("products.status", "live")
        if (selectedBrands.length > 0) q = q.in("products.brand", selectedBrands)
        if (selectedSubcategories.length > 0) q = q.in("products.sub_category", selectedSubcategories)
        if (selectedColors.length > 0) q = q.in("products.color_filter", selectedColors)
        if (selectedStyleTypes.length > 0) q = q.in("products.style_type", selectedStyleTypes)
        if (sortValue === "newest") q = q.order("products(created_at)", { ascending: false })
        else if (sortValue === "price-low") q = q.order("products(price)", { ascending: true })
        else if (sortValue === "price-high") q = q.order("products(price)", { ascending: false })
        else q = q.order("sort_order", { ascending: true })
        q = q.range(currentOffset, currentOffset + batchSize - 1)
        const { data: productCategories, error } = await q
        ...
        if (productCategories && productCategories.length > 0) {
          ...
          const cfProductIds = newProductsData.filter((p: any) => p.use_cloudflare_images).map((p: any) => p.id)
          const cfImagesMap = new Map<string, any[]>()
          if (cfProductIds.length > 0) {
            const { data: cfImages } = await supabase.from("product_images_cf").select("*").in("product_id", cfProductIds).order("sort_order", { ascending: true })
            if (cfImages) {
              cfImages.forEach((img: any) => {
                if (!cfImagesMap.has(img.product_id)) cfImagesMap.set(img.product_id, [])
                cfImagesMap.get(img.product_id)!.push({ id: img.id, url: buildCfUrl(img.cf_image_id, "grid"), alt_text: img.alt_text, display_order: img.sort_order, color_name: img.color_name, color_hex: img.color_hex })
          ...
          const optimized = newProductsData.map((product: any) => {
            const images = product.use_cloudflare_images ? cfImagesMap.get(product.id) || [] : product.product_images || []
            const imagesByColor = new Map<string, any>()
            images.sort((a: any, b: any) => a.display_order - b.display_order).forEach((img: any) => {
              const colorKey = img.color_name || "default"
          ...
          const existingIds = new Set(products.map((p) => p.id))
          const unique = optimized.filter((p) => !existingIds.has(p.id))
          if (unique.length > 0) { setProducts((prev) => [...prev, ...unique]); setCurrentOffset((prev) => prev + productCategories.length) }
        }
      }
    } catch (err) {
      console.error("[v0] Exception loading more products:", err)
    } finally {
      setIsLoading(false)
    }
  }
      ...
      <div className="max-w-[1920px] mx-auto">
        <div className="mb-6 px-4">
          ...
          <p className="text-[13px] text-[#111111] leading-relaxed mb-6">
            Explore our curated collection of men&apos;s accessories, featuring everything from refined belts and
            wallets to statement watches and contemporary jewellery. Each piece is carefully selected to elevate any
            outfit with effortless style.
          </p>
        </div>
        ...
        {sortedProducts.length === 0 ? (
          ...
          <>
            ...
            {hasMore && <div ref={loadMoreTriggerRef} className="h-20 flex items-center justify-center" />}
          </>
        )}
      </div>
    </div>
