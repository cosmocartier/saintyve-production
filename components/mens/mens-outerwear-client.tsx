...
interface MensOuterwearClientProps {
...
export function MensOuterwearClient({
  ...
  useEffect(() => {
    const restoreState = async () => {
      const savedScrollPosition = sessionStorage.getItem("mensOuterwearPageScrollPosition")
      const savedProductCount = sessionStorage.getItem("mensOuterwearPageLoadedCount")
      ...
      if (savedProductCount && savedScrollPosition) {
        ...
        sessionStorage.removeItem("mensOuterwearPageScrollPosition")
        sessionStorage.removeItem("mensOuterwearPageLoadedCount")
      }
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      sessionStorage.setItem("mensOuterwearPageScrollPosition", window.scrollY.toString())
      sessionStorage.setItem("mensOuterwearPageLoadedCount", products.length.toString())
    }
  ...
  const buildFilterUrl = () => {
    ...
    return `/mens/outerwear${params.toString() ? `?${params.toString()}` : ""}`
  }
      ...
      if (sortValue === "recommended") {
        ...
        if (smartProducts && smartProducts.length > 0) {
          ...
          if (unique.length > 0) {
            ...
            setCurrentOffset((prev) => prev + unique.length)
          }
        }
      } else {
        const supabase = createBrowserClient()
      ...
      <div className="max-w-[1920px] mx-auto">
        <div className="mb-6 px-4">
          ...
          <p className="text-[13px] text-[#111111] leading-relaxed mb-6">
            Explore our curated collection of men&apos;s outerwear, featuring everything from classic coats and tailored
            blazers to contemporary jackets and seasonal layers. Each piece is carefully selected to blend timeless style
            with modern comfort.
          </p>
        </div>
